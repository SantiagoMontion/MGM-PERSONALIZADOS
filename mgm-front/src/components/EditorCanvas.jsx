...
const EditorCanvas = forwardRef(function EditorCanvas(
  ...
  const onTransformEnd = () => {
    ...
    const finalScaleX = liveScaleX < 0 ? -clampScale(liveScaleX) : clampScale(liveScaleX);
    const finalScaleY = liveScaleY < 0 ? -clampScale(liveScaleY) : clampScale(liveScaleY);
    const w = imgBaseCm.w * Math.abs(finalScaleX);
    const h = imgBaseCm.h * Math.abs(finalScaleY);
    setImgTx({
      x_cm: centerX - w / 2,
      y_cm: centerY - h / 2,
      scaleX: finalScaleX,
