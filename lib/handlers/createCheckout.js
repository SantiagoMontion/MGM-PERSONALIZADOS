...
/** Storefront: `checkoutCreate` está deprecado; `cartCreate` devuelve `checkoutUrl` válido (evita "Checkout token is required…"). */
const CART_CREATE_FOR_CHECKOUT_MUTATION = `
...
function buildLineItemAttributes({ rid, designSlug }) {
  ...
  if (rid) {
    customAttributes.push({ key: '_rid', value: rid });
  }
  if (designSlug) {
    customAttributes.push({ key: '_design_slug', value: designSlug });
  }
  return customAttributes;
}

async function callShopifyCheckoutCreate({ variantId, quantity, rid, designSlug }) {
  ...
  if (customAttributes.length) {
    line.attributes = customAttributes;
  }

  const variables = { lines: [line] };
  ...
  try {
    response = await fetch(endpoint, {
      ...
      body: JSON.stringify({ query: CART_CREATE_FOR_CHECKOUT_MUTATION, variables }),
    ...
    if (controller?.signal?.aborted) {
      ...
      timeoutError.step = 'shopify_storefront_cart_create';
      throw timeoutError;
    }
    throw err;
  } finally {
  ...
  const checkoutId = typeof cart.id === 'string' ? cart.id : null;
  ...
  return { ok: true, checkoutUrl, checkoutId, requestId };
}

function buildCartFallbackUrl(domain, variantNumericId, quantity) {
  if (!domain || !variantNumericId) return '';
  const normalizedDomain = domain.replace(/^https?:\/\//i, '').replace(/\/+$/, '');
  if (!normalizedDomain) return '';
  const qty = parseQuantity(quantity);
  return `https://${normalizedDomain}/cart/${variantNumericId}:${qty}`;
}
...
async function handleRealHandler(req, res, diagId) {
  await runWithLenientCors(req, res, async () => {
    ...
    if (!result?.ok) {
      ...
      const fallbackUrl =
        buildCartFallbackUrl(domain, variantNumeric, quantity)
...
/** Orígenes separados por coma o espacio (ej. https://personalizados.notmid.ar,https://www.…) */
...
/**
 * Lista efectiva para validar el header Origin del navegador.
 * - CHECKOUT_ALLOWED_ORIGINS (Vercel): obligatorio en producción para el dominio del front.
 * - FRONTEND_ORIGIN: un solo origen opcional.
 * - VERCEL_URL: URL de preview de Vercel (https://…).
 * - En desarrollo se añaden localhost habituales (Vite, etc.).
 */
function getCheckoutAllowedOrigins() {
  const out = new Set();

  for (const piece of parseOriginsList(process.env.CHECKOUT_ALLOWED_ORIGINS)) {
    const n = normalizeOrigin(piece);
    if (n) out.add(n);
  }
...
function isAllowedOrigin(req) {
  ...
  const allowed = getCheckoutAllowedOrigins();
...
export default async function createCheckout(req, res) {
  ...
  /* Preflight primero: si el allowlist fallaba antes, el navegador veía OPTIONS 403 y bloqueaba el POST. */
  ...
  if (!isAllowedOrigin(req)) {
    sendJsonWithCors(req, res, 403, { ok: false, error: 'forbidden', diagId });
    return;
  }
