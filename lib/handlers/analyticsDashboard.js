...
function buildDeviceBreakdown(deviceByRid, sets) {
  ...
  const totalCart = mobile.added_to_cart + desktop.added_to_cart;
  ...
  mobile.cart_share = formatRate(mobile.added_to_cart, totalCart);
  desktop.cart_share = formatRate(desktop.added_to_cart, totalCart);
  mobile.cart_from_visit_rate = formatRate(mobile.added_to_cart, mobile.visitors);
  desktop.cart_from_visit_rate = formatRate(desktop.added_to_cart, desktop.visitors);
  ...
  return {
    ...
    desktop,
    cart_split: {
      mobile: mobile.added_to_cart,
      desktop: desktop.added_to_cart,
      mobile_percent: mobile.cart_share,
      desktop_percent: desktop.cart_share,
    },
    completion_split: {
...
async function fetchTrackEvents(supabase, fromIso, toIso) {
  const withBudget = (promise, ms) => Promise.race([
    ...
    new Promise((_, reject) => {
      setTimeout(() => reject(Object.assign(new Error('query_budget_exceeded'), { code: '57014' })), ms);
    }),
  ]);

  try {
    const rpcResult = await withBudget(
      supabase.rpc('analytics_fetch_track_events', {
        p_from: fromIso,
        p_to: toIso,
      }),
      20000,
    ...
    // RPC lento/missing: caemos a tabla.
    if (error?.code !== '57014' && error?.message !== 'query_budget_exceeded') {
      // continue to table
    }
  }

  try {
    const tableResult = await withBudget(
      supabase
        .from('track_events')
    ...
    return {
      rows: [],
      error: { message: error?.message || 'query_budget_exceeded', code: error?.code || '57014' },
      source: 'failed',
    };
  }
}
...
export async function analyticsDashboard(req, res) {
  ...
  // El sync de Shopify es pesado y dispara 504 en Vercel (sin headers CORS → el browser
  // lo reporta como CORS). Por defecto NO bloqueamos el dashboard; el front tiene
  // /api/analytics/sync-purchases. Solo corre si sync=1|force y skip_sync!=1.
  let purchaseSync = null;
  const syncParam = String(req.query?.sync ?? '').toLowerCase();
  const wantSync = syncParam === '1' || syncParam === 'force';
  const skipSync = String(req.query?.skip_sync ?? (wantSync ? '' : '1')) === '1';
  if (!skipSync && wantSync) {
    const rangeDays = Math.max(1, Math.ceil((toDate.getTime() - fromDate.getTime()) / (24 * 60 * 60 * 1000)));
    try {
      const syncPromise = maybeAutoSyncPaidOrders({
        sinceDays: Math.min(14, rangeDays),
        force: syncParam === 'force',
      });
      const timeoutPromise = new Promise((resolve) => {
        setTimeout(() => resolve({ ok: true, skipped: true, reason: 'sync_budget_exceeded' }), 4000);
      });
      purchaseSync = await Promise.race([syncPromise, timeoutPromise]);
    } catch (error) {
      logger.warn?.('analytics_dashboard_auto_sync_failed', {
        diagId,
        error: error?.message || error,
      });
      purchaseSync = { ok: false, error: error?.message || String(error) };
    }
  } else {
    purchaseSync = { ok: true, skipped: true, reason: skipSync ? 'skip_sync' : 'sync_not_requested' };
  }
  ...
  for (const row of rows) {
    ...
    // Prefer client-stamped device_type. Skip purchase_completed when it has no
    // device (Shopify webhook) — otherwise newest-first locks every buyer as desktop.
    if (rid) {
      const explicitDevice = readExtraString(row.extra, 'device_type');
      if (explicitDevice === 'mobile' || explicitDevice === 'desktop') {
        if (!deviceByRid.has(rid)) {
          deviceByRid.set(rid, explicitDevice);
        }
      } else if (!deviceByRid.has(rid) && eventName !== PURCHASE) {
  ...
  // Orphan purchases (webhook-only rid): fall back to UA / desktop default.
  for (const row of rows) {
    const rid = normalizeRid(row?.rid);
    if (!rid || deviceByRid.has(rid)) continue;
  ...
  const purchaseEvents = rows.filter((row) => row?.event_name === PURCHASE);

  const mockupEngagedSet = new Set([
    ...mockupViewSet,
    ...mockupOptionsSet,
    ...mockupClickSet,
  ]);
  const mockupPurchases = intersectCount(purchaseSet, mockupEngagedSet);
  ...
  sendJson(res, 200, {
    ...
    /** Embudo del personalizador embebido (Home, ruta /). Es el flujo principal de compra. */
    checkout_funnel: {
      review: reviewSet.size,
      cart: cartSet.size,
      purchases: purchaseSet.size,
      rate_review_to_cart: formatRate(reviewToCart, reviewSet.size),
      rate_visit_to_purchase: formatRate(visitToPurchase, visitSet.size),
    },
    /** Solo sesiones que pasaron por /mockup (página standalone, flujo legacy). */
    mockup_funnel: {
      view: mockupViewSet.size,
      options: mockupOptionsSet.size,
      clicks: mockupClickSet.size,
      purchase: mockupPurchases,
    },
    ...
    purchases_detail: purchaseEvents.slice(0, 20).map((row) => ({
      rid: row.rid,
      created_at: row.created_at,
      order_id: readExtraString(row.extra, 'shopify_order_id'),
      order_name: readExtraString(row.extra, 'shopify_order_name'),
      total_price: readExtraString(row.extra, 'total_price'),
      source: readExtraString(row.extra, 'source'),
    })),
