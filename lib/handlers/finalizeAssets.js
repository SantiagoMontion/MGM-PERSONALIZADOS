...
export default async function finalizeAssets(req, res) {
  ...
  try {
    ...
    console.error('finalize-assets upload', { diagId, error: err?.message || err, code: err?.code, size: err?.size, limit: err?.limit });
  ...
  try {
    ...
    console.error('finalize-assets prints_upsert', { diagId, error: err?.message || err });
    try {
