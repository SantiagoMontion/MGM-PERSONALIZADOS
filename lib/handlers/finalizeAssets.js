...
function resolvePreviewUrlFromPayload(payload) {
  const candidates = [
    payload?.mockupPublicUrl,
    payload?.mockup_public_url,
    payload?.preview_url,
    payload?.previewUrl,
    payload?.mockupUrl,
    payload?.mockup_url,
...
function resolveStraightEdges(payload, renderDescriptor) {
  if (!payload && !renderDescriptor) return false;
  return Boolean(
    ...
    || renderDescriptor?.straightEdges === true
    || renderDescriptor?.straight_edges === true
    || renderDescriptor?.options?.straightEdges === true
    || renderDescriptor?.options?.straight_edges === true,
  );
}
...
export default async function finalizeAssets(req, res) {
  ...
  const isStraightEdges = resolveStraightEdges(payload, renderDescriptor);
  ...
  const payloadPreviewUrl = resolvePreviewUrlFromPayload(payload);
  let previewUrl = payloadPreviewUrl;
  if (previewPath) {
    const { data: previewPublicData } = supabase.storage.from(OUTPUT_BUCKET).getPublicUrl(previewPath);
    const derivedPreviewUrl = previewPublicData?.publicUrl || null;
    if (!previewUrl) previewUrl = derivedPreviewUrl;
  }
  ...
  try {
    const designNameForPrints = String(designNameSource || '').trim() || null;
    ...
    logger.error('finalize-assets prints_upsert', { diagId, error: error?.message || error });
    console.error('[CRITICAL DB ERROR] Fallo al insertar en prints:', JSON.stringify(error, null, 2));
    try {
