...
import { DEFAULT_UPLOADS_BUCKET, normalizeOriginalReference } from '../_lib/uploads.js';
...
function extractOriginalReference(payload, job) {
  const sources = [payload?.assets, payload?.upload, payload?.job, payload];
  ...
  for (const source of sources) {
    if (!originalObjectKey) {
      originalObjectKey = pickFirstString(source, [
        'originalObjectKey',
        'original_object_key',
        'objectKey',
        'object_key',
    ...
    if (!originalBucket) {
      originalBucket = pickFirstString(source, [
        'originalBucket',
        'original_bucket',
        'bucket',
      ]);
    ...
    if (!originalUrl) {
      originalUrl = pickFirstString(source, [
        'originalUrl',
        'original_url',
        'designUrl',
        'design_url',
  ...
  const fallbackUrl = typeof job?.file_original_url === 'string' ? job.file_original_url : '';
  const normalized = normalizeOriginalReference({
    originalObjectKey,
    originalBucket,
    originalUrl: originalUrl || fallbackUrl,
  });

  return {
    originalObjectKey: normalized.originalObjectKey,
    originalBucket: normalized.originalObjectKey
      ? normalized.originalBucket || DEFAULT_UPLOADS_BUCKET
      : '',
    originalUrl: normalized.originalUrl || fallbackUrl,
  };
}
...
export default async function finalizeAssets(req, res) {
  ...
  const originalReference = extractOriginalReference(payload, job);
  ...
  const previewPath = previewUpload?.path ? (previewUpload.path.startsWith('outputs/') ? previewUpload.path : `outputs/${previewUpload.path}`) : (pdfPath.startsWith('outputs/') ? pdfPath : `outputs/${pdfPath}`);
  ...
  const originalBucketForPayload = originalReference.originalObjectKey
    ? originalReference.originalBucket || DEFAULT_UPLOADS_BUCKET
    : '';
  const originalUrlForPayload = originalReference.originalUrl || job.file_original_url || '';
  ...
  if (originalReference.originalObjectKey) {
    assetsPayload.original_object_key = originalReference.originalObjectKey;
    assetsPayload.originalObjectKey = originalReference.originalObjectKey;
  }
  if (originalBucketForPayload) {
    assetsPayload.original_bucket = originalBucketForPayload;
    assetsPayload.originalBucket = originalBucketForPayload;
  }
  if (originalUrlForPayload) {
    assetsPayload.original_url = originalUrlForPayload;
    assetsPayload.originalUrl = originalUrlForPayload;
  }
    ...
    if (job.id) {
      await supabase.from('job_events').insert({
        ...
        detail: {
          ...
          pdf_path: pdfPath,
          signed_url_expires_in: uploadResult.expiresIn ?? null,
          ...(originalReference.originalObjectKey || originalUrlForPayload
            ? {
                original_object_key: originalReference.originalObjectKey || null,
                original_bucket: originalBucketForPayload || null,
                original_url: originalUrlForPayload || null,
              }
            : {}),
