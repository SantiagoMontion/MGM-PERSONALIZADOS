  ...
  try {
    ...
    try {
      const originalAsset = await fetchOriginalAsset({
        ...
        originalUrl: originalReferenceUrl,
        rid,
        diagId,
      });
      ...
      if (originalBuffer && originalBuffer.length) {
        ...
        originalMime = detectedOriginal?.mime || null;
      }
    } catch (originalError) {
      if (originalError?.code === 'original_not_found') {
        logger.warn('publish-product original_not_found', {
          ...
          originalUrl: originalReferenceUrl || null,
        });
        const payload = {
          ok: false,
          code: 'original_not_found',
          message: 'No encontramos el archivo original para generar el PDF. Revisá la subida e intentá nuevamente.',
          ...(diagId ? { diagId } : {}),
          ...(rid ? { rid } : {}),
          detail: {
            originalObjectKey: originalAssetRef.originalObjectKey || null,
        ...
        logPresentBodyFields({ body, status: 422, diagId });
    ...
    if (PDF_ON_PURCHASE_ONLY && !originalAssetRef.originalObjectKey && !originalUrlForReference) {
      logger.warn('publish-product original_reference_missing', { diagId, rid });
      warnings.push({
        code: 'original_reference_missing',
        message: 'No encontramos la referencia al archivo original. Seguimos con la publicación.',
      });
    }
    try {
      if (!PDF_ON_PURCHASE_ONLY) {
        ...
        if (!originalBuffer || !originalBuffer.length) {
          throw Object.assign(new Error('original_not_found'), { code: 'original_not_found' });
        }
        ...
        const finalWidthCm = pdfResult.info?.pageWidthCm ?? (widthValue != null ? widthValue + (marginPerSideMm * 2) / 10 : null);
        const finalHeightCm = pdfResult.info?.pageHeightCm ?? (heightValue != null ? heightValue + (marginPerSideMm * 2) / 10 : null);
      ...
      } else if (pdfError?.code === 'suspicious_output') {
        logger.warn('publish-product suspicious_output', {
          diagId,
          rid,
          inBytes: pdfError?.in || null,
          outBytes: pdfError?.out || null,
        });
        warnings.push({
          code: 'suspicious_output',
          message: 'El PDF generado no pasó los controles de calidad. Seguimos con la publicación.',
          detail: {
            inBytes: pdfError?.in || null,
            outBytes: pdfError?.out || null,
          },
