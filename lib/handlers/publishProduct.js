...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const privateMetafieldValue = privateFromBody ? 'true' : 'false';
    ...
    const normalizedMetafields = [
      ...sanitizedMetafields,
      { namespace: 'custom', key: 'private', type: 'boolean', value: privateMetafieldValue },
    ];
    ...
    {
      const mf = Array.isArray(productInput.metafields) ? productInput.metafields : [];
      const idx = mf.findIndex((m) => m.namespace === 'custom' && m.key === 'private');
      const privateMeta = {
        namespace: 'custom',
        key: 'private',
        type: 'boolean',
        value: privateFromBody ? 'true' : 'false',
      };

      if (idx >= 0) mf[idx] = { ...mf[idx], ...privateMeta };
      else mf.push(privateMeta);

      productInput.metafields = mf;
    }
    ...
    const shouldSetPrivateTrue = privateFromBody;
    const shouldSetPrivateFalse = !privateFromBody;

    if (shouldSetPrivateTrue || shouldSetPrivateFalse) {
      const metafieldValue = privateMetafieldValue;
