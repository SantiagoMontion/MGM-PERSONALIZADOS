...
function normalizeMaterialForTemplateSuffix(material) {
...
function resolveTemplateSuffixByMaterial(material) {
  const normalized = normalizeMaterialForTemplateSuffix(material);
  if (!normalized) return null;
  if (normalized === 'glasspad') return 'glasspads';
  if (normalized === 'classic' || normalized === 'pro') return 'mousepads';
  if (normalized === 'alfombra') return 'alfombras';
  return null;
}
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const { key: productTypeKey } = pickProductType(body.productType);
    ...
    const materialLabel = resolveMaterialLabelFromProductType(body.productType);
    const productTypeLabel = resolveProductLabel({ productTypeRaw: body.productType, materialLabel });

    const materialForTemplateSuffix = materialLabel;
    const materialForTemplateSuffixNormalized = normalizeMaterialForTemplateSuffix(materialForTemplateSuffix);
    const templateSuffix = resolveTemplateSuffixByMaterial(materialForTemplateSuffix);

    try {
      logger.info('shopify_template_suffix_resolved', {
        materialRaw: materialForTemplateSuffix || null,
        materialNormalized: materialForTemplateSuffixNormalized || null,
