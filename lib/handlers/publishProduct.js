...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    try {
      ...
      const resolvedPrintSourceUrl = rawPrintSourceUrl || originalAssetRef.originalUrl || originalUrl;
      if (!PDF_ON_PURCHASE_ONLY && (rawPrintDataUrl || resolvedPrintSourceUrl)) {
        ...
        } else if (resolvedPrintSourceUrl) {
          const response = await fetch(resolvedPrintSourceUrl);
          if (!response.ok) {
