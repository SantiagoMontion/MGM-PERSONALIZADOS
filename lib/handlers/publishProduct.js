...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    // GraphQL ProductInput NO acepta "variants" ni "images".
    const productInput = {
      title,
      status: 'ACTIVE',
    ...
    if (!hasCreatedImages) {
      try {
        ...
        const firstAttachmentImage = productImagesInput.find(
          (image) => typeof image?.attachment === 'string' && image.attachment,
        );
        ...
        if (productIdForRest && adminToken && shopDomain) {
          ...
          let body = null;
          if (mockupSrc) {
            body = { image: { src: mockupSrc, alt: firstSrcImage?.altText || imageAlt } };
          } else if (firstAttachmentImage?.attachment) {
            const filename = firstAttachmentImage?.filename || `${designName || title || 'Mockup'}.png`;
            body = {
              image: {
                attachment: firstAttachmentImage.attachment,
                filename,
              },
            };
          }
