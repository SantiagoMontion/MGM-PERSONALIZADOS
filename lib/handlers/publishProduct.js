...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const privateExplicit = typeof body.private === 'boolean' ? body.private : null;
    const isPrivateExplicit = privateExplicit !== null
      ? privateExplicit
    ...
    try {
      console.log('[audit:shopify:create]', {
        ...
        mockupSrcPresent: Boolean(
          (typeof body?.imageSrc === 'string' && body.imageSrc.trim())
          || (typeof body?.mockupUrl === 'string' && body.mockupUrl.trim())
          || (typeof body?.mockupPublicUrl === 'string' && body.mockupPublicUrl.trim()),
        ),
      });
    } catch (_) {
      // noop
    ...
    // ===== Adjuntar imagen por REST Admin con src (mockup publico) =====
    ...
    if (!hasCreatedImages) {
      ...
      const imageSrcCandidates = [
        ...
        typeof firstSrcImage?.src === 'string' ? firstSrcImage.src.trim() : '',
        typeof mockupUrlRaw === 'string' ? mockupUrlRaw.trim() : '',
      ];
      const rawImageSrc = imageSrcCandidates.find((candidate) => candidate) || '';
      const normalizedImageSrc = typeof rawImageSrc === 'string' ? rawImageSrc.trim() : '';
      const hasImageSrc = Boolean(normalizedImageSrc);
      const isHttpImageSrc = hasImageSrc && /^https?:\/\//i.test(normalizedImageSrc);
      const imageSrc = isHttpImageSrc ? normalizedImageSrc : '';
      ...
      let imagePostOk = false;
      let imagePostStatus = null;

      if (imageSrc && productIdForRest && adminToken && shopDomain) {
        ...
        const bodyPayload = {
          image: {
            src: imageSrc,
            alt: firstSrcImage?.altText || imageAlt || title,
          },
        };
        try {
          const imgRes = await fetch(endpoint, { method: 'POST', headers, body: JSON.stringify(bodyPayload) });
          imagePostStatus = imgRes.status;
          if (imgRes.ok) {
            ...
            try {
              console.log('[audit:shopify:image:error]', {
                diagId: req?.mgmDiagId || null,
                status: imgRes.status,
                src: imageSrc,
                body: typeof errTxt === 'string' ? errTxt.slice(0, 400) : '',
              });
            } catch (_) {
              // noop
            }
    ...
    if (shouldSetPrivateTrue || shouldSetPrivateFalse) {
      ...
      try {
        ...
        try {
          console.log('[audit:shopify:metafield]', {
            diagId: req?.mgmDiagId || null,
            private: metafieldValue === 'true',
            ok: metafieldOk,
