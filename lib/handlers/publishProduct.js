...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const imageAlt = typeof body.imageAlt === 'string' && body.imageAlt.trim()
      ...
      : designName || title;
    const privateFromBody = body?.private === true;
    ...
    for (const entry of rawMetafields) {
      ...
      sanitized.value = typeof entry.value === 'string' ? entry.value : String(entry.value);
      sanitizedMetafields.push(sanitized);
    }
    const metafieldsWithoutPrivate = sanitizedMetafields.filter(
      (entry) => entry.namespace !== 'custom' || entry.key !== 'private',
    );
    const privateMetafieldValue = privateFromBody ? 'true' : 'false';
    const normalizedMetafields = [
      ...metafieldsWithoutPrivate,
