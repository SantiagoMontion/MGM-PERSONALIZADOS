...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    if (shouldSetPrivateTrue || shouldSetPrivateFalse) {
      ...
      try {
        ...
        if (!metafieldOk) {
          ...
          try {
            console.info('product_private_metafield_set', {
              productId: productIdForVariants,
              value: metafieldValue,
              requestId: metafieldRequestId || null,
            });
    ...
    try {
      ...
      if (rawPrintDataUrl || rawPrintSourceUrl) {
        ...
        try {
          ...
          console.error('publish-product prints_upsert', { error: printsErr?.message || printsErr });
        }
      }
    } catch (pdfError) {
      console.error('publish-product pdf_generation_failed', pdfError);
    ...
    const publishedFlag = publishResult?.ok === true;
    try {
