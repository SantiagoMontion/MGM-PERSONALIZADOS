...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const materialResolvedRaw = typeof body.materialResolved === 'string' ? body.materialResolved.trim() : '';
    const materialOptionRaw = typeof body?.options?.material === 'string' ? body.options.material.trim() : '';
    const materialRaw = typeof body.material === 'string' ? body.material.trim() : '';
    const materialLabel = materialResolvedRaw
      || materialOptionRaw
      || materialRaw
      || (productTypeKey === 'glasspad' ? 'Glasspad' : 'Classic');
    // (NO volver a normalizar material acá; confiar en payload.materialResolved que ya viene canónico)

    const materialForTemplateSuffix = materialResolvedRaw || materialOptionRaw || materialRaw;
    ...
    const title = (incomingTitle || fallbackTitle).slice(0, 254);
