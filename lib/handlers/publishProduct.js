...
import { resolveCustomerPriceFromNetBase, ULTRA_FINAL_LIST_PRICE } from '../pricing/equilibrium.js';
import { applyShopifyListPriceMarkup } from '../pricing/shopifyListPrice.js';
import { resolveShopifySalePricing } from '../pricing/siteWideShopifyDiscount.js';
import { buildPersonalizedProductSeo } from '../seo/productSeo.js';
import {
  findOutOfStockMaterial,
  MATERIAL_OUT_OF_STOCK_REASON,
} from '../materials/outOfStock.js';

const DEFAULT_VENDOR = 'NOTMID';
...
/** job_id estable para prints + SKU Shopify + properties del carrito. */
function ensurePublishJobId(body) {
  const candidates = [
    body?.jobId,
    body?.job_id,
    body?.options?.jobId,
...
/** Solo si MGM_PUBLISH_VERBOSE_LOG=1 (no loguear payloads completos en producción). */
function logPublishVerbose(...args) {
  if (process.env.MGM_PUBLISH_VERBOSE_LOG === '1') {
    console.log(...args);
  }
}
...
async function waitUrlOk(url, tries = 10, delayMs = 350) {
  if (typeof url !== 'string' || !url) return false;
  const totalTries = Math.max(1, Number.isFinite(tries) && tries > 0 ? Math.floor(tries) : 1);
  const baseDelay = Math.max(0, Number.isFinite(delayMs) && delayMs > 0 ? delayMs : 0);
  for (let attempt = 0; attempt < totalTries; attempt += 1) {
    try {
      const response = await fetch(url, {
        method: 'HEAD',
        redirect: 'follow',
        cache: 'no-store',
        headers: { 'user-agent': 'MGMPublish/1.0 (+mockup-head)' },
      });
      if (response?.ok) {
        return true;
      }
      ...
      try {
        logger?.warn?.('mockup_head_attempt_failed', {
          url,
          attempt,
...
function buildMockupObjectPath({ filename = '', buffer = null } = {}) {
  const date = new Date();
  const y = date.getUTCFullYear();
  const m = String(date.getUTCMonth() + 1).padStart(2, '0');
  const d = String(date.getUTCDate()).padStart(2, '0');
  const extFromFilename = typeof filename === 'string' && filename.includes('.')
    ? filename.split('.').pop().trim().toLowerCase()
    : '';
  const ext = extFromFilename && /^[a-z0-9]{2,5}$/.test(extFromFilename)
    ? extFromFilename
    : 'jpg';
  const fingerprint = computeImageHash(buffer).slice(0, 24);
  return `shopify-mockups/${y}/${m}/${d}/${fingerprint}.${ext}`;
}

async function uploadMockupToOutputs({ buffer, filename, mimeType }) {
  if (!Buffer.isBuffer(buffer) || !buffer.length) return null;
  const supabase = getSupabaseAdmin();
  const objectPath = buildMockupObjectPath({ filename, buffer });
  const contentType = typeof mimeType === 'string' && mimeType.trim()
    ? mimeType.split(';')[0].trim().toLowerCase()
    : 'image/jpeg';

  const { error: uploadError } = await supabase.storage.from(OUTPUT_BUCKET).upload(objectPath, buffer, {
    upsert: true,
    cacheControl: '3600',
    contentType,
    metadata: {
      private: 'false',
      createdBy: 'publish_product',
      assetType: 'shopify_mockup',
    },
  });

  if (uploadError) {
    const error = new Error('mockup_output_upload_failed');
    error.code = 'mockup_output_upload_failed';
    error.cause = uploadError;
    throw error;
  }

  const { data: publicData } = supabase.storage.from(OUTPUT_BUCKET).getPublicUrl(objectPath);
  return {
    path: objectPath,
    publicUrl: sanitizeShopifyImageUrl(publicData?.publicUrl || ''),
  };
}
...
function parseIncomingPrice(value) {
  if (value && typeof value === 'object') {
    const amountCandidate = value.amount ?? value.value ?? value.price ?? null;
    return parseIncomingPrice(amountCandidate);
  }
...
function pickProductType(raw) {
  ...
  if (v.includes('alfombr')) {
    return { key: 'alfombra', label: 'Alfombra' };
  }
  if (v.includes('lamp') || v.includes('ilumin')) {
    return { key: 'lampara', label: 'Lámpara' };
  }
...
/** Ruta relativa al bucket outputs (p. ej. pdf-2026-03/foo.pdf) desde URL pública o firmada de Supabase. */
function parseOutputsStoragePathFromSupabasePublicUrl(rawUrl, bucket) {
  if (typeof rawUrl !== 'string') return '';
  const trimmed = rawUrl.trim();
  if (!trimmed) return '';
  const b = String(bucket || OUTPUT_BUCKET).replace(/\/+$/, '');
  const tryNormalizeRel = (relRaw) => {
    let rel = String(relRaw || '').split('?')[0].split('#')[0].replace(/^\/+/, '');
    try {
      rel = decodeURIComponent(rel);
    } catch {
      // keep raw
    }
    if (rel.toLowerCase().startsWith(`${b.toLowerCase()}/`)) {
      rel = rel.slice(b.length + 1);
    }
  ...
  const publicNeedle = `/storage/v1/object/public/${b}/`;
  const signNeedle = `/storage/v1/object/sign/${b}/`;
  let idx = pathname.indexOf(publicNeedle);
  let needleLen = publicNeedle.length;
  if (idx === -1) {
    idx = pathname.indexOf(signNeedle);
    needleLen = signNeedle.length;
  }
  if (idx !== -1) {
    const rel = tryNormalizeRel(pathname.slice(idx + needleLen));
    if (rel) return rel;
  }

  const lowerPath = pathname.toLowerCase();
  const bucketPos = lowerPath.indexOf(`/${b.toLowerCase()}/`);

  if (bucketPos !== -1) {
    const rel = tryNormalizeRel(pathname.slice(bucketPos + 1));
    if (rel) return rel;
  }

  if (/^https?:\/\//i.test(trimmed)) return '';

  return tryNormalizeRel(pathname) || '';
}

/**
 * Confirma que el PDF cliente ya está en Storage antes de registrar `prints`.
 * Sin esto se crean filas huérfanas (Shopify OK, descarga HTML en /busqueda).
 */
async function resolveExistingOutputsPdfObject(supabase, bucket, objectPath) {
  const path = String(objectPath || '').replace(/^\/+/, '');
  if (!path || !supabase) {
    return { ok: false, sizeBytes: null, reason: 'missing_path' };
  }
  ...
  if (signErr || !signed?.signedUrl) {
    return {
      ok: false,
      sizeBytes: null,
      reason: signErr?.message || 'object_not_found',
    };
  }
  ...
  try {
    ...
    const hit = Array.isArray(listed)
      ? listed.find((entry) => entry?.name === base)
      : null;
    const rawSize = hit?.metadata?.size;
    if (Number.isFinite(Number(rawSize)) && Number(rawSize) > 0) {
      sizeBytes = Number(rawSize);
    }
  } catch {
    // size is best-effort; existence already confirmed via signed URL
  }

  return { ok: true, sizeBytes, reason: null };
}
...
function resolveStableIdempotencyKey(body = {}) {
  const provided = typeof body?.shopifyIdempotencyKey === 'string' ? body.shopifyIdempotencyKey.trim() : '';
  if (provided) return provided.slice(0, 255);
  const designHash = typeof body?.designHash === 'string' ? body.designHash.trim().toLowerCase() : '';
  if (!/^[a-f0-9]{64}$/.test(designHash)) return null;
  const material = String(body?.material || body?.materialResolved || body?.options?.material || 'material')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, '-');
  return `publish:${designHash}:${material || 'material'}`.slice(0, 255);
}
...
/** Misma lógica que mgm-front `applyHistoricalAndInflationMarkup` (60100 → transfer ~72.000) */
const ULTRA_TRANSFER_PRICE = 60100;
const LAMP_TRANSFER_PRICE = 130000;
...
function mapCalculatorMaterial(material, productTypeKey) {
  ...
  if (productTypeKey === 'pro') return 'Pro';
  if (productTypeKey === 'alfombra') return 'Alfombra';
  if (productTypeKey === 'lampara') return 'Lampara';
  ...
  if (raw.includes('lamp') || raw.includes('ilumin')) return 'Lampara';
...
function finalizeServerListPricing(netBasePrice, mode) {
...
function calculateTransferPrice({ material, productTypeKey, widthCm, heightCm }) {
  ...
  if (mode === 'Lampara') {
    return finalizeServerListPricing(LAMP_TRANSFER_PRICE, mode);
  }
  if (mode === 'Glasspad') {
    ...
    return finalizeServerListPricing(transferPriceRounded, mode);
  }
  if (mode === 'Ultra') {
    return {
      transfer: ULTRA_FINAL_LIST_PRICE,
      normal: ULTRA_FINAL_LIST_PRICE,
    };
  }

  const rollo = ROLLO_PRICING[mode];
  if (!rollo) {
    return finalizeServerListPricing(0, mode);
  }
  ...
  const surcharge = 2000 + (mode === 'Clasic' ? 2000 : 0);
...
function formatCustomerMaterialLabel(materialLabel, isCircular = false) {
  const normalizedMaterial = typeof materialLabel === 'string' ? materialLabel.trim() : '';
...
function buildUltraTitle({ designName }) {
  const normalizedName = typeof designName === 'string' ? designName.trim() : '';
  const name = normalizedName || 'Personalizado';
  return `Mousepad Serie Ultra ${name} | Custom`;
}

function buildDefaultTitle({ productTypeLabel, designName, measurement, materialLabel, isCircular = false }) {
  const parts = [];
  if (designName) parts.push(designName);
  if (measurement) parts.push(measurement);
  const displayMaterial = formatCustomerMaterialLabel(materialLabel || productTypeLabel, isCircular);
  if (displayMaterial) parts.push(displayMaterial);
  if (!parts.length && productTypeLabel) parts.push(productTypeLabel);
  const core = `${parts.join(' ').trim() || 'Custom'} | Custom`;
  if (productTypeLabel === 'Alfombra') {
    return `Alfombra ${core}`;
  }
  if (productTypeLabel === 'Lámpara') {
    return `Lámpara ${core}`;
  }
  return `Mousepad ${core}`;
}

function normalizeCustomerProductTitle(value) {
...
function normalizeMaterialLabel(rawMaterial) {
  const raw = typeof rawMaterial === 'string' ? rawMaterial.trim() : '';
...
function resolveMaterialLabel({ body, productTypeRaw }) {
  const explicitMaterial = body?.material ?? body?.materialResolved ?? body?.options?.material;
  const explicitRaw = typeof explicitMaterial === 'string' ? explicitMaterial.trim() : '';
  if (explicitRaw) {
    return normalizeMaterialLabel(explicitRaw);
  }
  const fallbackType = String(productTypeRaw || body?.productType || body?.options?.productType || '').trim();
  if (fallbackType) {
...
function resolveProductLabel({ productTypeRaw, materialLabel }) {
  const normalizedType = String(productTypeRaw || '').trim().toLowerCase();
  if (normalizedType.includes('glass')) return 'Glasspad';
  if (normalizedType.includes('lamp') || normalizedType.includes('ilumin')) return 'Lámpara';
  if (materialLabel === 'Alfombra') return 'Alfombra';
  if (materialLabel === 'PRO') return 'PRO';
  return 'Mousepad';
}

function resolveIncomingProductType(body) {
  if (!body || typeof body !== 'object') return '';
  const candidates = [
    body.productType,
    body?.options?.productType,
  ];
  for (const candidate of candidates) {
    if (typeof candidate === 'string' && candidate.trim()) {
...
function buildMetaDescription({ productTypeLabel, designName, widthCm, heightCm, materialLabel, productTitle }) {
  const measurement = buildMeasurement(widthCm, heightCm);
  return buildPersonalizedProductSeo({
    material: materialLabel || productTypeLabel,
    designName,
    measurement,
    productTitle,
  }).description;
...
function resolveProductSeo({ body, materialLabel, designName, measurementLabel, productTitle }) {
  const incomingTitle = typeof body?.seoTitle === 'string'
    ? body.seoTitle
    : typeof body?.seo_title === 'string'
      ? body.seo_title
      : '';
  const incomingDescription = typeof body?.seoDescription === 'string'
    ? body.seoDescription
  ...
  return buildPersonalizedProductSeo({
    ...
    productTitle,
    seoTitle: incomingTitle,
    seoDescription: incomingDescription,
  });
}
...
async function executeProductCreate({ input, filename, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;

  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(PRODUCT_CREATE_MUTATION, { input }, idempotencyKey);
...
async function rollbackCreatedProduct({ productId, idempotencyKey, diagId, reason }) {
  const normalizedProductId = typeof productId === 'string' ? productId.trim() : '';
  if (!normalizedProductId) {
    return { ok: false, reason: 'missing_product_id' };
  }
  try {
    const resp = await shopifyAdminGraphQL(PRODUCT_DELETE_MUTATION, {
      input: { id: normalizedProductId },
    }, idempotencyKey);
    const requestId = logRequestId('product_delete_request', resp, {
    ...
    if (!resp.ok || userErrors.length || !payload?.deletedProductId) {
      try {
        logger.error('publish_product_rollback_failed', {
          diagId,
          productId: normalizedProductId,
          reason,
          status: resp.status,
      ...
      return { ok: false, status: resp.status, requestId, userErrors };
    }
    try {
      logger.warn('publish_product_rollback_success', {
        diagId,
        productId: normalizedProductId,
    ...
    return { ok: true, requestId, deletedProductId: payload.deletedProductId };
  } catch (rollbackError) {
    try {
      logger.error('publish_product_rollback_exception', {
        diagId,
        productId: normalizedProductId,
        reason,
    ...
    return { ok: false, reason: 'rollback_exception', message: rollbackError?.message || String(rollbackError) };
  }
}

async function executeProductStatusUpdate({ productId, status = 'ACTIVE', maxAttempts = 2, metafields, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const input = { id: normalizedId, status: normalizedStatus };
      if (Array.isArray(metafields) && metafields.length) {
        input.metafields = metafields;
      }
      const variables = { input };
      const resp = await shopifyAdminGraphQL(PRODUCT_STATUS_UPDATE_MUTATION, variables, idempotencyKey);
...
function buildBulkVariantUpdateVariables({ productId, input }) {
  ...
  // compareAtPrice debe viajar también en el fallback bulk; si no, Shopify deja 0/vacío.
  if (typeof input?.compareAtPrice === 'string' && input.compareAtPrice) {
    variant.compareAtPrice = input.compareAtPrice;
  } else if (input?.compareAtPrice === null) {
  ...
  if (typeof input?.inventoryPolicy === 'string' && input.inventoryPolicy) {
    variant.inventoryPolicy = input.inventoryPolicy;
  }
  return {
    productId,
    variants: [variant],
...
async function executeProductVariantUpdate({ input, productId, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  // Preferir bulk cuando hay compare-at: productVariantUpdate está deprecado y a veces
  // actualiza price pero no persiste compareAtPrice en APIs nuevas.
  let useBulkMutation = Boolean(
    typeof productId === 'string'
    && productId
    && (typeof input?.compareAtPrice === 'string' || input?.compareAtPrice === null),
  );
  ...
  for (let attempt = 0; attempt < maxAttempts;) {
    ...
    try {
      const resp = await shopifyAdminGraphQL(mutation, variables, idempotencyKey);
...
async function executeProductVariantRestUpdate({ variantId, payload, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  ...
  if (payload?.compareAtPrice != null && payload.compareAtPrice !== '') {
    variantPayload.compare_at_price = payload.compareAtPrice;
  } else if (payload?.compareAtPrice === null) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdmin(`variants/${legacyId}.json`, { method: 'PUT', body }, idempotencyKey);
...
async function executeProductVariantQuery({ productId, first = 1, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  const variables = {
    id: productId,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(PRODUCT_VARIANT_QUERY, variables, idempotencyKey);
...
async function executeProductCreateMedia({ productId, media, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  const variables = { productId, media };
  const semanticRetryDelayMs = 2000;

  const collectMediaSemanticErrors = (json) => {
    const payload = json?.data?.productCreateMedia;
    const mediaUserErrors = Array.isArray(payload?.mediaUserErrors)
      ? sanitizeUserErrors(payload.mediaUserErrors)
      : [];
    const legacyUserErrors = Array.isArray(payload?.userErrors)
      ? sanitizeUserErrors(payload.userErrors)
      : [];
    const mediaErrors = Array.isArray(payload?.media)
      ? payload.media
        .filter((entry) => entry && typeof entry === 'object')
        .flatMap((entry) => Array.isArray(entry.mediaErrors) ? entry.mediaErrors : [])
        .map((entry) => ({
          field: Array.isArray(entry?.field) ? entry.field : [],
          message: typeof entry?.message === 'string' ? entry.message : '',
          code: typeof entry?.code === 'string' ? entry.code : '',
        }))
        .filter((entry) => Boolean(entry.message || entry.code))
      : [];
    return {
      mediaUserErrors,
      legacyUserErrors,
      mediaErrors,
      hasErrors: mediaUserErrors.length > 0 || legacyUserErrors.length > 0 || mediaErrors.length > 0,
    };
  };

  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(PRODUCT_CREATE_MEDIA_MUTATION, variables, idempotencyKey);
      ...
      if (resp.ok) {
        const semantic = collectMediaSemanticErrors(json);
        ...
        if (attempt < maxAttempts - 1) {
          try {
            logger.warn('product_create_media_retry_semantic', {
              attempt: attempt + 1,
              requestId: requestId || null,
              productId,
              mediaUserErrors: semantic.mediaUserErrors,
              userErrors: semantic.legacyUserErrors,
              mediaErrors: semantic.mediaErrors,
            });
          } catch {}

          await sleep(semanticRetryDelayMs * (attempt + 1));
          continue;
        }
...
async function executeLocationsQuery({ first = 5, maxAttempts = 3, idempotencyKey } = {}) {
  const attempts = [];
  let lastError = null;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(LOCATIONS_QUERY, variables, idempotencyKey);
...
async function executeInventoryItemUpdate({ id, input, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(INVENTORY_ITEM_UPDATE_MUTATION, variables, idempotencyKey);
...
async function executeInventorySetOnHandQuantities({ input, maxAttempts = 3, idempotencyKey }) {
  const attempts = [];
  let lastError = null;
  const variables = { input };
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdminGraphQL(INVENTORY_SET_ON_HAND_QUANTITIES_MUTATION, variables, idempotencyKey);
...
async function executeInventoryLevelsSet({ inventoryItemId, locationId, available, maxAttempts = 3, idempotencyKey }) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdmin('inventory_levels/set.json', { method: 'POST', body }, idempotencyKey);
...
function buildPublishFingerprintTag(body = {}) {
  const rawFingerprint = typeof body?.publishFingerprint === 'string' ? body.publishFingerprint.trim().toLowerCase() : '';
  if (rawFingerprint) {
    return `mgm_fp:${rawFingerprint.replace(/[^a-z0-9|x:_-]+/g, '-')}`.slice(0, 60);
  }
  const designHash = typeof body?.designHash === 'string' ? body.designHash.trim().toLowerCase() : '';
  if (!/^[a-f0-9]{64}$/.test(designHash)) return '';
  const material = normalizeMaterialLabel(body?.material || body?.materialResolved || body?.options?.material || 'Classic')
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, '-');
  return `mgm_fp:${designHash.slice(0, 16)}-${material || 'classic'}`.slice(0, 60);
}
...
async function findExistingProductByFingerprintTag({ tag, idempotencyKey }) {
  const normalizedTag = typeof tag === 'string' ? tag.trim() : '';
  if (!normalizedTag) return null;
  const query = `tag:${escapeShopifySearchValue(normalizedTag)}`;
  const resp = await shopifyAdminGraphQL(PRODUCT_BY_TAG_QUERY, { query, first: 1 }, idempotencyKey);
  const requestId = logRequestId('product_search_by_tag', resp, { tag: normalizedTag });
  const json = await resp.json().catch(() => null);
  if (!resp.ok) {
    try {
      logger.warn('product_search_by_tag_warning', { tag: normalizedTag, status: resp.status, requestId: requestId || null, body: json });
    } catch {}
    return null;
  }
  const errors = Array.isArray(json?.errors) ? json.errors : [];
  if (errors.length) {
    try {
      logger.warn('product_search_by_tag_graphql_errors', { tag: normalizedTag, requestId: requestId || null, errors: formatGraphQLErrors(errors) });
    } catch {}
    return null;
  }
  const node = json?.data?.products?.nodes?.[0];
  if (!node || typeof node !== 'object' || !node.id) return null;
  const variantNode = Array.isArray(node?.variants?.nodes) ? node.variants.nodes[0] : null;
  const variantLegacyId = variantNode?.legacyResourceId ?? variantNode?.legacy_resource_id;
  const variantGid = typeof variantNode?.id === 'string' ? variantNode.id : null;
  const variantId = variantLegacyId != null
    ? String(variantLegacyId)
    : variantGid;
  return {
    id: node.id,
    legacyResourceId: node.legacyResourceId || null,
    handle: typeof node.handle === 'string' ? node.handle : '',
    onlineStoreUrl: typeof node.onlineStoreUrl === 'string' ? node.onlineStoreUrl : '',
    status: typeof node.status === 'string' ? node.status : null,
    variantId: variantId || null,
    variantAdminId: variantGid || null,
    requestId: requestId || null,
  };
}
...
const PRODUCT_BY_TAG_QUERY = `query ProductsByQuery($query: String!, $first: Int!) {
...
const PRODUCT_UPDATE_IMAGES_MUTATION = `mutation ProductUpdate($input: ProductInput!) {
...
const PRODUCT_METAFIELDS_SET_MUTATION = `mutation metafieldsSet($metafields: [MetafieldsSetInput!]!) {
...
async function stagedUploadImage({ filename, buffer, mimeType, maxUploadAttempts = 3, idempotencyKey }) {
  ...
  const resp = await shopifyAdminGraphQL(STAGED_UPLOADS_CREATE_MUTATION, { input }, idempotencyKey);
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const shouldRetryUpload =
      attempt < attempts - 1
      && (
        (uploadResp.status >= 500 && uploadResp.status < 600)
        || uploadResp.status === 408
        || uploadResp.status === 429
        || (uploadResp.status === 400 && buffer.length > 4 * 1024 * 1024)
      );
...
function buildCheckoutUrlFromVariantId(variantId) {
  const rawVariantId = typeof variantId === 'string' ? variantId.trim() : '';
  if (!rawVariantId) return '';
  const numericVariantId = extractLegacyId(rawVariantId, 'ProductVariant') || rawVariantId;
  const shopBase = (process.env.SHOPIFY_PUBLIC_BASE || process.env.SHOPIFY_STORE_DOMAIN || '').trim();
  if (!shopBase) return '';
  const normalizedBase = /^https?:\/\//i.test(shopBase)
    ? shopBase
    : `https://${shopBase}`;
  const base = normalizedBase.replace(/\/+$/, '');
  return `${base}/cart/${encodeURIComponent(String(numericVariantId))}:1`;
}
...
function buildRecoverablePublishResponse(snapshot = {}, error = null) {
  const warnings = Array.isArray(snapshot?.warnings) ? snapshot.warnings : [];
  const warningMessages = warnings
    .map((entry) => (entry && typeof entry.message === 'string' ? entry.message : null))
    .filter(Boolean);
  const errorMessage = typeof error?.message === 'string' ? error.message : 'No se pudo completar la post-publicación.';
  return {
    ok: true,
    recovered: true,
    recoveryReason: 'post_create_non_fatal_error',
    productId: snapshot.productId || snapshot.productAdminId || null,
    productAdminId: snapshot.productAdminId || null,
    productHandle: snapshot.productHandle || null,
    variantId: snapshot.variantId || null,
    variantAdminId: snapshot.variantAdminId || null,
    productUrl: snapshot.productUrl || null,
    publicUrl: snapshot.productUrl || null,
    url: snapshot.url || snapshot.productUrl || null,
    status: snapshot.status || null,
    visibility: snapshot.visibility || null,
    warnings: [...warnings, { code: 'post_create_warning', message: errorMessage }],
    warningMessages: [...warningMessages, errorMessage],
  };
}
...
export async function publishProduct(req, res) {
  ...
  let recoverableProductSnapshot = null;
  ...
  try {
    ...
    const publishJobId = ensurePublishJobId(body);
    body.jobId = publishJobId;
    body.job_id = publishJobId;
    logPublishVerbose('📩 publish body (verbose):', {
      designHash: typeof body.designHash === 'string' ? `${body.designHash.slice(0, 12)}…` : null,
      material: body.material ?? body.materialResolved,
      mode: body.mode,
      widthCm: body.widthCm,
      heightCm: body.heightCm,
    });

    const shopifyIdempotencyKey = resolveStableIdempotencyKey(body);

    const privateFromBody =
      body?.private === true
      || body?.isPrivate === true
      || body?.mode === 'private'
      || (Array.isArray(body?.metafields)
        && body.metafields.some((m) =>
          m?.namespace === 'custom'
          && m?.key === 'private'
          && (m?.value === true || m?.value === 'true' || m?.value === 1 || m?.value === '1')));

    try {
      logPublishVerbose('[audit:metafield-private]', { private: privateFromBody });
    } catch (_) {
      // noop
    }

    const mockupDataUrlRaw = typeof body.mockupDataUrl === 'string' ? body.mockupDataUrl.trim() : '';
    const mockupUrlRaw = typeof body.mockupUrl === 'string' ? body.mockupUrl.trim() : '';
    let filename = typeof body.filename === 'string' && body.filename ? body.filename : '';
    const warnings = [];
    const rawImages = Array.isArray(body?.images) ? body.images : [];
    const requestedImages = rawImages
      .map((entry) => {
        if (!entry || typeof entry !== 'object') return null;
        const attachment = typeof entry.attachment === 'string' ? entry.attachment.trim() : '';
        const srcRaw = typeof entry.src === 'string' ? entry.src.trim() : '';
        const src = sanitizeShopifyImageUrl(srcRaw);
        if (!attachment && !src) return null;
        const filename = typeof entry.filename === 'string' ? entry.filename.trim() : '';
        const altText = typeof entry.altText === 'string' ? entry.altText.trim() : '';
        const imagePayload = {};
        if (attachment) imagePayload.attachment = attachment;
        if (src) imagePayload.src = src;
        if (filename) imagePayload.filename = filename;
        if (altText) imagePayload.altText = altText;
        return imagePayload;
      })
      .filter(Boolean);
    ...
    let mockupSrcOnly = false;

    if (mockupDataUrlRaw) {
      if (!mockupDataUrlRaw.startsWith('data:')) {
        return res.status(400).json({ ok: false, reason: 'invalid_mockup_dataurl' });
      }
      const rawBase64 = (mockupDataUrlRaw.split(',')[1] || '').trim();
      if (!rawBase64) {
        return res.status(400).json({ ok: false, reason: 'invalid_mockup_dataurl' });
      }
      ...
      const meta = parseDataUrlMeta(mockupDataUrlRaw);
      mimeType = meta?.mimeType || mimeType;
      imageBuffer = Buffer.from(sanitizedBase64, 'base64');
    } else if (mockupUrlRaw) {
      const headReady = await waitUrlOk(mockupUrlRaw, 10, 350);
      if (!headReady) {
        mockupSrcOnly = true;
        try {
          logger?.warn?.('mockup_head_not_ready', { url: mockupUrlRaw });
        } catch {}
      } else {
        let fetchResponse;
        try {
          fetchResponse = await fetch(mockupUrlRaw, {
            method: 'GET',
            redirect: 'follow',
            headers: { 'user-agent': 'MGMPublish/1.0 (+mockup-get)' },
          });
        } catch (fetchErr) {
          mockupSrcOnly = true;
          try {
            logger?.warn?.('mockup_fetch_failed_head_ready', {
              url: mockupUrlRaw,
              message: typeof fetchErr?.message === 'string' ? fetchErr.message : String(fetchErr),
            });
          } catch {}
        }
        ...
        if (fetchResponse && !fetchResponse.ok) {
          mockupSrcOnly = true;
          try {
            logger?.warn?.('mockup_fetch_response_not_ok', {
              url: mockupUrlRaw,
        ...
        if (fetchResponse?.ok) {
          ...
          const headerMime = typeof fetchResponse.headers?.get === 'function'
            ? fetchResponse.headers.get('content-type')
            : fetchResponse.headers && typeof fetchResponse.headers['content-type'] === 'string'
              ? fetchResponse.headers['content-type']
              : '';
          const normalizedMime = typeof headerMime === 'string' ? headerMime.split(';')[0].trim() : '';
          if (normalizedMime) {
      ...
      warnings.push({
        code: 'missing_mockup',
        message: 'No se recibió mockup válido. Se continuará sin imagen de producto.',
      });
      try {
        logger.warn('product_mockup_missing_warning', {
          reason: 'missing_mockup',
          continueWithoutImage: true,
        });
      } catch {}
    }

    if ((!imageBuffer || !imageBuffer.length) && !mockupSrcOnly) {
      mockupSrcOnly = true;
      warnings.push({
        code: 'invalid_mockup',
        message: 'El mockup no se pudo leer. Se continuará sin imagen de producto.',
      });
      try {
        logger.warn('product_mockup_invalid_warning', {
          reason: 'invalid_mockup',
          continueWithoutImage: true,
        });
      } catch {}
    }
    ...
    if (!filename) {
      filename = mimeType.includes('png') ? 'mockup.png' : 'mockup.jpg';
    }
    if (!mimeType) {
      mimeType = 'image/jpeg';
    }

    let publishMockupUrl = sanitizeShopifyImageUrl(mockupUrlRaw);
    if (imageBuffer?.length) {
      try {
        const uploadedMockup = await uploadMockupToOutputs({ buffer: imageBuffer, filename, mimeType });
        if (uploadedMockup?.publicUrl) {
          publishMockupUrl = sanitizeShopifyImageUrl(uploadedMockup.publicUrl);
        }
        await sleep(2000);
      } catch (uploadErr) {
        return res.status(502).json({
          ok: false,
          reason: 'mockup_output_upload_failed',
          message: 'No se pudo preparar el mockup final en outputs para Shopify.',
    ...
    if (publishMockupUrl) {
      const isPublishMockupReachable = await waitUrlOk(publishMockupUrl, 3, 700);
      if (!isPublishMockupReachable) {
        return res.status(422).json({
          ok: false,
          reason: 'mockup_public_url_unreachable',
          message: 'La URL pública del mockup no es accesible desde Shopify (HEAD timeout/rechazo).',
          mockupUrl: publishMockupUrl,
        });
      }
    }

    const designNameRaw = typeof body.designName === 'string' ? body.designName.trim() : '';
    if (designNameRaw && projectNameContainsForbiddenWord(designNameRaw)) {
      return res.status(400).json({
        ok: false,
    ...
    const designName = designNameRaw || 'Personalizado';
    const productTypeRaw = resolveIncomingProductType(body);
    logPublishVerbose('Categoría detectada:', productTypeRaw);
    const { key: productTypeKey } = pickProductType(productTypeRaw);
    ...
    const materialLabel = resolveMaterialLabel({ body, productTypeRaw });
    const productTypeLabel = resolveProductLabel({ productTypeRaw, materialLabel });
    const outOfStock = findOutOfStockMaterial({
      material: materialLabel,
      productType: productTypeRaw || productTypeKey || productTypeLabel,
    });
    if (outOfStock) {
      return res.status(400).json({
        ok: false,
    ...
    const normalizedShape = String(
      body?.shape
      || body?.options?.shape
      || ((body?.isCircular === true || body?.options?.isCircular === true) ? 'circle' : 'rounded_rect'),
    ).trim().toLowerCase();
    const isCircularShape = normalizedShape === 'circle';
    const titleHint = typeof body?.title === 'string' ? body.title : '';
    const tagsHint = Array.isArray(body?.tags)
      ? body.tags.map((tag) => String(tag || '').trim().toLowerCase()).filter(Boolean)
      : typeof body?.tags === 'string'
        ? body.tags.split(',').map((tag) => String(tag || '').trim().toLowerCase()).filter(Boolean)
        : [];
    const hasRectoHint =
      /(?:custom-recto|\brecto\b)/i.test(titleHint)
      || tagsHint.includes('shape-recto')
      || tagsHint.includes('recto');
    const isStraightEdges = !isCircularShape && (
      body?.straightEdges === true
      || body?.straight_edges === true
      || body?.options?.straightEdges === true
      || body?.options?.straight_edges === true
      || hasRectoHint
    );
    body.shape = isCircularShape ? 'circle' : 'rounded_rect';
    body.isCircular = isCircularShape;
    body.options = {
      ...(body.options || {}),
      shape: body.shape,
      isCircular: body.isCircular,
      straightEdges: isStraightEdges,
    };

    const incomingTitle = normalizeCustomerProductTitle(typeof body.title === 'string' ? body.title : '');
    const fallbackTitle = productTypeLabel === 'Glasspad'
      ? buildGlasspadTitle({ designName: designNameRaw, measurement: measurementLabel })
      : materialLabel === 'Ultra'
        ? buildUltraTitle({ designName: designNameRaw })
        : buildDefaultTitle({
          productTypeLabel,
          designName: designNameRaw,
          measurement: measurementLabel,
          materialLabel: productTypeLabel === 'Mousepad' ? materialLabel : productTypeLabel,
          isCircular: isCircularShape,
        });
    const title = (incomingTitle.includes('| Custom') ? incomingTitle : fallbackTitle).slice(0, 254);
    /** Plantilla de tema según título (serie); si no hay señal → producto predeterminado. */
    const templateSuffix = resolveThemeTemplateSuffix({ title, material: materialLabel });
    try {
      logger.info('shopify_template_suffix_resolved', {
        title: title || null,
        material: materialLabel || null,
        template_suffix: templateSuffix || null,
      });
    } catch {}
    const designNameForPdf = designNameRaw || title;

    const requestedPriceTransfer = parseIncomingPrice(body.priceTransfer ?? body.price_transfer);
    const requestedPriceNormal = parseIncomingPrice(body.priceNormal ?? body.price_normal ?? body.price);
    const frontendPrice = isFiniteNumber(requestedPriceTransfer) && requestedPriceTransfer > 0
      ? requestedPriceTransfer
      : (isFiniteNumber(requestedPriceNormal) && requestedPriceNormal > 0 ? requestedPriceNormal : null);

    let priceTransfer = 0;
    let priceTransferSource = 'missing';

    if (isFiniteNumber(requestedPriceTransfer) && requestedPriceTransfer > 0) {
      priceTransfer = requestedPriceTransfer;
      priceTransferSource = 'request_transfer';
    } else if (isFiniteNumber(requestedPriceNormal) && requestedPriceNormal > 0) {
      priceTransfer = requestedPriceNormal;
      priceTransferSource = 'request_normal';
    } else {
      ...
      const calculatorTransferPrice = isFiniteNumber(calculatorPricing?.transfer)
        ? calculatorPricing.transfer
        : null;
      if (isFiniteNumber(calculatorTransferPrice) && calculatorTransferPrice > 0) {
        priceTransfer = applyShopifyListPriceMarkup(calculatorTransferPrice, materialLabel);
        priceTransferSource = 'calculator';
      }
    }

    if (frontendPrice && frontendPrice > 0) {
      priceTransfer = frontendPrice;
      priceTransferSource = isFiniteNumber(requestedPriceTransfer) && requestedPriceTransfer > 0
        ? 'request_transfer_forced'
        : 'request_normal_forced';
    }
    ...
    const imageAlt = typeof body.imageAlt === 'string' && body.imageAlt.trim()
      ...
      : designName || title;
    const rawMetafields = Array.isArray(body?.metafields) ? body.metafields : [];
    const sanitizedMetafields = [];
    for (const entry of rawMetafields) {
      if (!entry || typeof entry !== 'object') continue;
      const namespace = typeof entry.namespace === 'string' ? entry.namespace : '';
      const key = typeof entry.key === 'string' ? entry.key : '';
      if (!namespace || !key) continue;
      const sanitized = { namespace, key };
      if (typeof entry.type === 'string' && entry.type.trim()) {
        sanitized.type = entry.type.trim();
      }
      if (entry.value == null) continue;
      const valueStr = typeof entry.value === 'string' ? entry.value : String(entry.value);
      if (namespace === 'custom' && key === 'private') {
        continue;
      }
      sanitized.value = valueStr;
      sanitizedMetafields.push(sanitized);
    }
    const normalizedMetafields = [...sanitizedMetafields];
    body.metafields = normalizedMetafields;

    let stagedImage = null;
    if (imageBuffer?.length) {
      try {
        stagedImage = await stagedUploadImage({ filename, buffer: imageBuffer, mimeType, idempotencyKey: shopifyIdempotencyKey });
      } catch (err) {
    ...
    // Usar el precio ya normalizado que nos pasó el edge (respeta PRO/Classic/Glasspad)
    const basePrice = isFiniteNumber(priceTransfer) ? Math.max(priceTransfer, 0) : 0;
    const salePricing = resolveShopifySalePricing({
      listPrice: basePrice,
      material: materialLabel,
      widthCm,
      heightCm,
    });
    const computedPrice = salePricing.price;
    const priceValue = formatMoney(computedPrice) || '0.00';
    const compareAtPriceValue = salePricing.compareAtPrice != null
      ? (formatMoney(salePricing.compareAtPrice) || null)
      : null;

    if (!Number.isFinite(computedPrice) || computedPrice <= 0 || priceValue === '0.00') {
      return res.status(400).json({
        ok: false,
    ...
    try {
      logger.debug('variant_price_computed', {
        ...
        requestedPriceTransfer: isFiniteNumber(requestedPriceTransfer)
          ? requestedPriceTransfer
          : null,
        requestedPriceNormal: isFiniteNumber(requestedPriceNormal)
          ? requestedPriceNormal
          : null,
        basePrice,
        computedPrice,
        priceValue,
        compareAtPriceValue,
        discountApplied: salePricing.discountApplied,
        materialLabel,
        widthCm,
        heightCm,
    ...
    try {
      ...
      } = await executeLocationsQuery({ first: 5, idempotencyKey: shopifyIdempotencyKey });
    ...
    const sanitizedMockupUrl = sanitizeShopifyImageUrl(publishMockupUrl || mockupUrlRaw);
    const fallbackMediaSource = !stagedImage?.originalSource && mockupSrcOnly && sanitizedMockupUrl
      ? sanitizedMockupUrl
      : null;
    const productMediaInput = stagedImage?.originalSource
      ...
      : fallbackMediaSource
        ? [{ mediaContentType: 'IMAGE', originalSource: fallbackMediaSource, alt: imageAlt }]
        : [];
    const productImagesInput = requestedImages
      .map((image) => {
        if (image.altText) return image;
        return { ...image, altText: imageAlt };
      });

    // (sin variants ni images: GraphQL ProductInput no lo admite)
    const fingerprintTag = buildPublishFingerprintTag(body);
    const productTags = normalizeTags([
      ...(Array.isArray(body?.tags) ? body.tags : []),
      fingerprintTag,
    ]);

    const productSeo = resolveProductSeo({
      body,
      materialLabel,
      designName: designNameRaw || designName,
      measurementLabel,
      productTitle: title,
    });
    ...
    productInput.title = typeof productInput.title === 'string' && productInput.title.length
      ? productInput.title
      : title;

    if (fingerprintTag) {
      const existingProduct = await findExistingProductByFingerprintTag({
        tag: fingerprintTag,
        idempotencyKey: shopifyIdempotencyKey,
      });
      if (existingProduct?.id) {
        const existingUrl = existingProduct.onlineStoreUrl
          || buildProductUrl(existingProduct.handle || null)
          || null;
        let variantId = typeof existingProduct.variantId === 'string' && existingProduct.variantId.trim()
          ? existingProduct.variantId.trim()
          : null;
        let variantAdminId = typeof existingProduct.variantAdminId === 'string' && existingProduct.variantAdminId.trim()
          ? existingProduct.variantAdminId.trim()
          : null;
        if (!variantId) {
          const {
            resp: vqResp,
            json: vqJson,
          } = await executeProductVariantQuery({
            productId: existingProduct.id,
            first: 1,
            idempotencyKey: shopifyIdempotencyKey,
          });
          if (vqResp.ok) {
            const vNodes = Array.isArray(vqJson?.data?.product?.variants?.nodes)
              ? vqJson.data.product.variants.nodes
              : [];
            const v0 = vNodes[0];
            if (v0 && typeof v0 === 'object') {
              const leg = v0.legacyResourceId ?? v0.legacy_resource_id;
              variantAdminId = typeof v0.id === 'string' ? v0.id : variantAdminId;
              variantId = leg != null ? String(leg) : variantAdminId;
            }
          }
        }
        if (!variantId) {
          try {
            logger.warn('product_dedupe_missing_variant', {
              productAdminId: existingProduct.id,
              handle: existingProduct.handle || null,
              tag: fingerprintTag,
            });
          } catch {}
          return res.status(502).json({
            ok: false,
            reason: 'product_dedupe_missing_variant',
            message: 'Se encontró el producto por huella pero Shopify no devolvió una variante. Reintentá o revisá el producto en el admin.',
            productAdminId: existingProduct.id,
            productHandle: existingProduct.handle || null,
            visibility,
            warnings,
            warningMessages: warnings.map((entry) => entry?.message).filter(Boolean),
          });
        }
        const resolvedDedupeCheckoutUrl = buildCheckoutUrlFromVariantId(variantId);
        return res.status(200).json({
          ok: true,
          deduped: true,
          dedupeSource: 'shopify_search_before_create',
          productId: existingProduct.legacyResourceId || existingProduct.id,
          productAdminId: existingProduct.id,
          productHandle: existingProduct.handle || null,
          variantId,
          ...(variantAdminId ? { variantAdminId } : {}),
          productUrl: existingUrl,
          publicUrl: existingUrl,
          url: existingUrl,
          ...(resolvedDedupeCheckoutUrl
            ? { checkoutUrl: resolvedDedupeCheckoutUrl }
            : {}),
          status: existingProduct.status || null,
          visibility,
          warnings,
          warningMessages: warnings.map((entry) => entry?.message).filter(Boolean),
        });
    ...
    try {
      logPublishVerbose('[audit:shopify:create]', {
        diagId: req?.mgmDiagId || null,
        title: productInput.title,
        seoTitle: productInput.seo?.title || null,
        priceToSet: priceValue,
        templateSuffix,
        template_suffix: templateSuffix || null,
        mockupSrcPresent: Boolean(body?.mockupUrl),
      });
    } catch (_) {
      // noop
    }
    ...
    } = await executeProductCreate({ input: productInput, filename, idempotencyKey: shopifyIdempotencyKey });
    ...
    recoverableProductSnapshot = {
      ...meta,
      warnings: [...warnings],
      visibility,
      status: product?.status || null,
      productUrl: buildProductUrl(meta?.productHandle || null) || null,
      url: buildProductUrl(meta?.productHandle || null) || null,
    };
    ...
    const createdImageEdges = Array.isArray(product?.images?.edges) ? product.images.edges : [];
    const hasCreatedImages = createdImageEdges.some((edge) => edge && edge.node);
    const productIdForVariants = typeof product?.id === 'string' && product.id
      ? product.id
      : meta.productAdminId;

    const rollbackAndRespond = async (statusCode, payload, reason) => {
      const rollback = await rollbackCreatedProduct({
        productId: productIdForVariants,
        idempotencyKey: shopifyIdempotencyKey,
        diagId: req?.mgmDiagId || null,
        reason,
      });
      return res.status(statusCode).json({ ...payload, rollback });
    };
    ...
    const productGid = typeof product?.id === 'string' ? product.id : meta.productAdminId || null;
    const productLegacyId = extractLegacyId(
      product?.legacyResourceId ?? meta?.productId ?? productGid,
      'Product',
    );

    // ===== Adjuntar imagen por REST Admin con src (mockup público) =====
    let restImageAttached = false;
    if (!hasCreatedImages) {
      try {
        const firstSrcImage = productImagesInput.find((image) => typeof image?.src === 'string' && image.src);
        const mockupSrc = sanitizeShopifyImageUrl(firstSrcImage?.src || publishMockupUrl || mockupUrlRaw || '');
        try {
          logPublishVerbose('[audit:shopify:image]', {
            diagId: req?.mgmDiagId || null,
            title,
            priceToSet: priceValue,
            mockupSrcPresent: Boolean(mockupSrc),
          });
        } catch (_) {
          // noop
        }
        const adminToken = process.env.SHOPIFY_ADMIN_TOKEN;
        const shopConfig = process.env.SHOPIFY_SHOP || process.env.SHOPIFY_STORE_DOMAIN || '';
        const apiVersion = process.env.SHOPIFY_API_VERSION || '2024-04';
        const shopDomain = shopConfig.includes('.') ? shopConfig : `${shopConfig}.myshopify.com`;
        const productIdForRest = productLegacyId || (typeof productGid === 'string' ? productGid.split('/').pop() : '');

        if (productIdForRest && adminToken && shopDomain) {
          const baseUrl = `https://${shopDomain}/admin/api/${apiVersion}`;
          const endpoint = `${baseUrl}/products/${productIdForRest}/images.json`;
          const headers = {
            'Content-Type': 'application/json',
            'X-Shopify-Access-Token': adminToken,
          };
          if (mockupSrc) {
            const body = { image: { src: mockupSrc, alt: firstSrcImage?.altText || imageAlt } };
            const imgRes = await fetch(endpoint, { method: 'POST', headers, body: JSON.stringify(body) });
            if (imgRes.ok) {
              restImageAttached = true;
            } else {
              const errTxt = await imgRes.text().catch(() => '');
              try {
                logger?.warn?.('product_image_rest_attach_failed', { status: imgRes.status, errTxt });
              } catch {}
            }
          } else {
            try {
              logger?.warn?.('product_image_rest_missing_src');
            } catch {}
          }
        }
      } catch (e) {
        try {
          logger?.warn?.('product_image_rest_attach_exception', { message: e?.message || String(e) });
        } catch {}
      }
    }

    if (!hasCreatedImages && productImagesInput.length && !restImageAttached) {
      const firstSrcImage = productImagesInput.find((image) => typeof image.src === 'string' && image.src);
      const sanitizedFirstSrcImage = sanitizeShopifyImageUrl(firstSrcImage?.src || '');
      if (sanitizedFirstSrcImage) {
        try {
          const updateVariables = {
            input: {
              id: productIdForVariants,
              images: [{
                src: sanitizedFirstSrcImage,
                altText: firstSrcImage.altText || imageAlt,
              }],
            },
          };
          const updateResp = await shopifyAdminGraphQL(PRODUCT_UPDATE_IMAGES_MUTATION, updateVariables, shopifyIdempotencyKey);
          const updateRequestId = logRequestId('product_update_images_request', updateResp, {
            productId: meta.productAdminId || null,
          });
          const updateJson = await updateResp.json().catch(() => null);
          const updateUserErrors = Array.isArray(updateJson?.data?.productUpdate?.userErrors)
            ? updateJson.data.productUpdate.userErrors
            : [];
          if (!updateResp.ok || updateUserErrors.length) {
            try {
              logger.warn('product_update_images_warning', {
                requestId: updateRequestId || null,
          ...
          try {
            logger.warn('product_update_images_exception', {
              message: err?.message || String(err),
              productId: meta.productAdminId || null,
            });
          } catch {}
    ...
    if (!defaultVariant?.id || !defaultVariant?.inventoryItem?.id) {
      ...
      } = await executeProductVariantQuery({ productId: productIdForVariants, first: 1, idempotencyKey: shopifyIdempotencyKey });
    ...
    } = await executeProductVariantUpdate({ input: variantUpdateInput, productId: productIdForVariants, idempotencyKey: shopifyIdempotencyKey });
    ...
    if (!updatedVariant) {
      ...
      if (!hasVariantPayload) {
        return rollbackAndRespond(502, {
          ...
          visibility,
          ...meta,
        }, 'variant_update_payload_missing');
      }
      ...
      if (variantUserErrors.length) {
        ...
        if (hasGraphQLUserMissingScope) {
          ...
          }, 'variant_update_scope_missing_user_error');
        }
    ...
    try {
      logger.info(
        compareAtPriceValue
          ? `✅ Precio de variante actualizado a: ${priceValue} (compare-at: ${compareAtPriceValue}).`
          : `✅ Precio de variante actualizado correctamente a: ${priceValue}.`,
      );
    } catch {}
    ...
    try {
      ...
      } = await executeInventoryItemUpdate({ id: variantInventoryItemId, input: inventoryItemUpdateInput, idempotencyKey: shopifyIdempotencyKey });
    ...
    if (productMediaInput.length) {
      ...
      const mediaInitialDelayMs = 2000;

      let mediaAssociationOk = false;
      let mediaWarningPayload = null;

      try {
        await sleep(mediaInitialDelayMs);
        const {
          resp: mediaResp,
          json: mediaJson,
          requestId: mediaRequestId,
          attempts: mediaAttempts,
        } = await executeProductCreateMedia({ productId: productIdForVariants, media: productMediaInput, idempotencyKey: shopifyIdempotencyKey });
    ...
    if (!isActiveStatus(product)) {
      ...
      } = await executeProductStatusUpdate({
        productId: productIdForVariants,
        status: 'ACTIVE',
        metafields: normalizedMetafields,
        idempotencyKey: shopifyIdempotencyKey,
    ...
    try {
      ...
      if (rawPrintDataUrl || rawPrintSourceUrl) {
        ...
        const designHashForFilename = typeof body.designHash === 'string' ? body.designHash.trim().toLowerCase() : '';
        const pdfMetadata = {
          slug: pdfSlug,
        ...
        if (/^[a-f0-9]{64}$/.test(designHashForFilename)) {
          pdfMetadata.jobKey = designHashForFilename;
        }
        if (typeof body.jobId === 'string' && body.jobId.trim()) {
          pdfMetadata.jobId = body.jobId.trim();
        } else {
          pdfMetadata.jobId = publishJobId;
        }
        let pdfUpload;
        try {
        ...
        try {
          ...
          const jobIdForRecord = publishJobId;
          const printsDesignParts = [...new Set(
            [incomingTitle, designNameRaw, title]
              .map((s) => (typeof s === 'string' ? s.trim() : ''))
              .filter(Boolean),
          )];
          const designNameForPrints = printsDesignParts.length
            ? printsDesignParts.join(' | ').slice(0, 4000)
            : (typeof designNameForPdf === 'string' ? designNameForPdf.trim() || null : null);
          printsPayloadPrimary = {
            job_key: jobKey,
            bucket: OUTPUT_BUCKET,
            file_path: pdfUpload.path,
            file_name: pdfUpload.fileName || pdfUpload.path.split('/').pop(),
            slug: pdfSlug,
            design_name: designNameForPrints,
          ...
          if (printsPayloadPrimary) {
            try {
              const supabase = getSupabaseAdmin();
              const { error: retryErr } = await supabase.from('prints').upsert(
                printsPayloadPrimary,
                { onConflict: 'job_key', ignoreDuplicates: false },
              );
              if (retryErr) {
                logger.error('publish-product prints_upsert_retry_failed', { error: retryErr?.message || retryErr });
              }
            } catch (retryErr) {
              logger.error('publish-product prints_upsert_retry_failed', { error: retryErr?.message || retryErr });
            }
        ...
        const rawPdfPublicUrl = typeof body.pdfPublicUrl === 'string' ? body.pdfPublicUrl.trim() : '';
        const existingPdfPath = parseOutputsStoragePathFromSupabasePublicUrl(rawPdfPublicUrl, OUTPUT_BUCKET);
        if (existingPdfPath) {
          const supabase = getSupabaseAdmin();
          const existingPdf = await resolveExistingOutputsPdfObject(
            supabase,
            OUTPUT_BUCKET,
            existingPdfPath,
          );
          if (!existingPdf.ok) {
            const missingErr = new Error('client_pdf_missing_in_storage');
            missingErr.code = 'client_pdf_missing';
            missingErr.path = existingPdfPath;
            missingErr.detail = existingPdf.reason;
            throw missingErr;
          }

          const normalizedBg = normalizeBackgroundHex(background);
          const designHashBody = typeof body.designHash === 'string' ? body.designHash.trim().toLowerCase() : '';
          const imageHash = /^[a-f0-9]{64}$/.test(designHashBody)
            ? designHashBody
            : computeImageHash(Buffer.from(existingPdfPath, 'utf8'));
          const pdfSlug = slugifyName(designNameForPdf) || 'diseno';
          const jobKey = buildPrintsJobKey({
            slug: pdfSlug,
            widthCm: finalWidthCm ?? widthValue,
          ...
          const designNameForPrints = printsDesignParts.length
            ? printsDesignParts.join(' | ').slice(0, 4000)
            : (typeof designNameForPdf === 'string' ? designNameForPdf.trim() || null : null);
          const fileName = existingPdfPath.split('/').filter(Boolean).pop() || `${pdfSlug}.pdf`;
          const mockupPublic = typeof body.mockupPublicUrl === 'string' ? body.mockupPublicUrl.trim() : '';
          const { error: printsErrExisting } = await supabase.from('prints').upsert({
            job_key: jobKey,
            bucket: OUTPUT_BUCKET,
            file_path: existingPdfPath,
            file_name: fileName,
            slug: pdfSlug,
            design_name: designNameForPrints,
            width_cm: finalWidthCm ?? widthValue ?? null,
            ...
            bg_color: normalizedBg,
            job_id: jobIdForRecord || null,
            file_size_bytes: existingPdf.sizeBytes,
            image_hash: imageHash,
            ...(mockupPublic ? { preview_url: mockupPublic } : {}),
          }, { onConflict: 'job_key', ignoreDuplicates: false });
          if (printsErrExisting) {
            throw printsErrExisting;
          }
        } else if (rawPdfPublicUrl) {
          try {
            logger.warn('publish-product prints_skip_unparsed_pdf_url', {
              message: 'No se pudo derivar file_path desde pdfPublicUrl; revisá formato de URL (public/sign o ruta relativa al bucket).',
            });
          } catch {
            // noop
          }
          const missingErr = new Error('client_pdf_url_unparsed');
          missingErr.code = 'client_pdf_missing';
          throw missingErr;
        } else {
          const missingErr = new Error('client_pdf_public_url_missing');
          missingErr.code = 'client_pdf_missing';
          throw missingErr;
        }
      }
    } catch (pdfError) {
      logger.error('publish-product pdf_generation_failed', {
        error: pdfError?.message || pdfError,
        code: pdfError?.code || null,
        path: pdfError?.path || null,
        detail: pdfError?.detail || null,
      });
      const productIdForRollback = typeof meta?.productAdminId === 'string' && meta.productAdminId
        ? meta.productAdminId
        : (typeof product?.id === 'string' ? product.id : null);
      const rollbackResult = await rollbackCreatedProduct({
        productId: productIdForRollback,
        idempotencyKey: shopifyIdempotencyKey,
        diagId: req?.mgmDiagId || null,
        reason: pdfError?.code === 'client_pdf_missing' ? 'client_pdf_missing' : 'pdf_generation_failed',
      });
      ...
      if (pdfError?.code === 'client_pdf_missing') {
        return res.status(409).json({
          ok: false,
          reason: 'client_pdf_missing',
          message: 'El PDF de impresión no está disponible en el almacenamiento. Volvé a generar el diseño e intentá de nuevo.',
          path: pdfError.path || null,
          rollback: rollbackResult,
        });
      }
    ...
    const metafieldOwnerId = (() => {
      if (product && typeof product === 'object' && typeof product?.id === 'string' && product.id) {
        return product.id;
      }
      if (meta && typeof meta === 'object') {
        if (typeof meta.productAdminId === 'string' && meta.productAdminId) return meta.productAdminId;
        if (typeof meta.productId === 'string' && meta.productId) return meta.productId;
      }
      return null;
    })();
    const isPrivateRequest = body?.private === true
      || body?.mode === 'private'
      || body?.checkoutType === 'private';
    try {
      logPublishVerbose('[audit:private-flag]', {
        incomingPrivate: body?.private,
        mode: body?.mode,
        checkoutType: body?.checkoutType,
        willSet: isPrivateRequest,
      });
    } catch {}
    if (metafieldOwnerId) {
      try {
        const metafieldInputs = [
          {
            ...
            key: 'private',
            type: 'boolean',
            value: isPrivateRequest ? 'true' : 'false',
          },
          {
            ownerId: metafieldOwnerId,
            namespace: 'custom',
        ...
        const metafieldResp = await shopifyAdminGraphQL(PRODUCT_METAFIELDS_SET_MUTATION, {
        ...
        const metafieldUserErrors = sanitizeUserErrors(metafieldPayload?.userErrors);
        if (metafieldUserErrors.length) {
          try {
            logger.warn('product_private_metafield_user_errors', {
              productId: metafieldOwnerId,
        ...
        if (!metafieldResp.ok) {
          try {
            logger.warn('product_private_metafield_set_warning', {
              productId: metafieldOwnerId,
              status: metafieldResp.status,
              body: metafieldJson,
            });
          } catch {}
        }
      ...
      try {
        logger.warn('product_private_metafield_missing_owner', {
          willSet: isPrivateRequest,
        });
      } catch {}
    }

    const resolvedCheckoutUrl = buildCheckoutUrlFromVariantId(meta?.variantId);
    ...
    const responsePayload = {
      ...
      visibility,
      ...(resolvedCheckoutUrl ? { checkoutUrl: resolvedCheckoutUrl, url: resolvedCheckoutUrl } : {}),
    ...
    recoverableProductSnapshot = {
      ...
      visibility,
      status: product?.status || null,
      productUrl: responsePayload.productUrl || responsePayload.publicUrl || responsePayload.url || recoverableProductSnapshot?.productUrl || null,
      url: responsePayload.url || responsePayload.productUrl || recoverableProductSnapshot?.url || null,
    };
    ...
    if (recoverableProductSnapshot?.productAdminId || recoverableProductSnapshot?.productId) {
      return res.status(200).json(buildRecoverablePublishResponse(recoverableProductSnapshot, e));
    }
