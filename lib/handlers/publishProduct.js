...
import { resolveCustomerPriceFromNetBase } from '../pricing/equilibrium.js';
...
function calculateTransferPrice({ material, productTypeKey, widthCm, heightCm }) {
  ...
  if (mode === 'Ultra') {
    ...
    const transferWithBaseMarkup = transferPrice * 1.12;
    const roundedHistoricalTransfer = Math.round(transferWithBaseMarkup / 500) * 500;
    const transferWithInflation = roundedHistoricalTransfer * 1.07;
    const transferPriceRounded = Math.round(transferWithInflation / 500) * 500;
    return finalizeServerListPricing(transferPriceRounded, mode);
  }
  ...
  try {
    ...
    if (isFiniteNumber(requestedPriceTransfer) && requestedPriceTransfer > 0) {
      ...
      if (isFiniteNumber(calculatorTransferPrice) && calculatorTransferPrice > 0) {
        priceTransfer = calculatorTransferPrice;
    ...
    try {
      logger.info(`✅ Precio de variante actualizado correctamente a: ${priceValue}.`);
    } catch {}
