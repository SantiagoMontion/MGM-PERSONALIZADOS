...
function respondMissingMockup({ res, body, diagId, originalAssetRef, detail, hasMockupCandidate }) {
  logPresentBodyFields({ body, status: 400, diagId });
...
function logPresentBodyFields({ body, status, diagId }) {
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    let mockupUrl = typeof mockupPayload.mockupUrl === 'string' ? mockupPayload.mockupUrl.trim() : '';
    ...
    let rawDataUrl = typeof mockupPayload.dataUrl === 'string' ? mockupPayload.dataUrl.trim() : '';

    if (mockupSelection) {
      if (mockupSelection.type === 'url') {
        mockupUrl = mockupSelection.value;
      } else if (mockupSelection.type === 'dataurl') {
    ...
    logPresentBodyFields({ body, status: 200, diagId });
