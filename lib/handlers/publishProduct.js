...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const mockupUrl = typeof mockupPayload.mockupUrl === 'string' ? mockupPayload.mockupUrl.trim() : '';
    ...
    const hasMockupUrl = Boolean(mockupUrl);
    const hasUsableMockupDataUrl = !hasMockupUrl && hasRawDataUrl
      ? rawDataUrl.toLowerCase().startsWith('data:image')
      : false;
    ...
    if (!coverImageSource) {
      const payload = {
        ...
        message: 'Falta el mockup para publicar el producto.',
        ...(diagId ? { diagId } : {}),
      };
      if (stagedDataUrlError && !hasAnyUrlCandidate) {
        payload.detail = { reason: stagedDataUrlError };
      }
      return res.status(400).json(payload);
    }
    ...
    const sizeMmForPersist = widthCm != null && heightCm != null
    ...
    const designSlug = slugifyName(designNameForPdf) || 'diseno';
