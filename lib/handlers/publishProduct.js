...
function sanitizeShopifyImageUrl(rawUrl) {
  ...
  if (!trimmed) return '';
  let cleanUrl = trimmed.split('?')[0].trim();
  if (!cleanUrl) return '';

  // Decodificación profunda para eliminar capas acumuladas (%25 -> % -> carácter real).
  while (true) {
    let decoded;
    try {
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const sanitizedMockupUrl = sanitizeShopifyImageUrl(mockupUrlRaw);
    ...
    if (!hasCreatedImages) {
      try {
        ...
        const mockupSrc = sanitizeShopifyImageUrl(firstSrcImage?.src || mockupUrlRaw || '');
        try {
