  ...
  try {
    ...
    if (mockupDataUrlRaw) {
      ...
      try {
        ...
        return res.status(400).json({ ok: false, reason: 'mockup_fetch_failed', detail: fetchErr?.message || null });
      }
      if (!fetchResponse?.ok) {
        return res.status(400).json({
          ok: false,
          reason: 'mockup_fetch_failed',
          status: typeof fetchResponse?.status === 'number' ? fetchResponse.status : undefined,
        });
      }
    ...
    if (!Array.isArray(productInput.variants) || !productInput.variants.length || productInput.variants[0].price == null) {
      productInput.variants = [{ price: priceValue }];
    }
