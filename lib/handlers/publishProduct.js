...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    console.log('📩 BACKEND RECIBIÓ:', body);
    ...
    try {
      console.log('[audit:metafield-private]', { private: privateFromBody });
    } catch (_) {
      // noop
    }
    ...
    console.log('Categoría detectada:', productTypeRaw);
    ...
    try {
      console.log('[audit:shopify:create]', {
    ...
    if (!hasCreatedImages) {
      try {
        ...
        try {
          console.log('[audit:shopify:image]', {
            diagId: req?.mgmDiagId || null,
            title,
            priceToSet: priceValue,
    ...
    try {
      console.log('[audit:private-flag]', {
