...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const hasMockupDataUrl = hasRawDataUrl && rawDataUrl.toLowerCase().startsWith('data:image');
    const hasMockupUrl = Boolean(mockupUrl);

    if (!hasMockupUrl && !hasMockupDataUrl) {
      return res.status(400).json({
        ok: false,
        code: 'missing_mockup',
    ...
    } else if (hasRawDataUrl && !hasMockupUrl) {
      stagedDataUrlError = 'invalid_mockup_dataurl';
    }
