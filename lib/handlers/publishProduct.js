...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const incomingTitle = typeof body.title === 'string' ? body.title : '';
    const fallbackTitle = productTypeLabel === 'Glasspad'
      ...
      : buildDefaultTitle({
        ...
        materialLabel: productTypeLabel === 'Mousepad' ? materialLabel : '',
      });
    const title = (incomingTitle.includes('| MGM-EDITOR') ? incomingTitle : fallbackTitle).slice(0, 254);
