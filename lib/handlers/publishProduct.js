...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const productInput = {
      ...
      ...(productImagesInput.length ? { images: productImagesInput } : {}),
    };
    ...
    if (!hasCreatedImages && requestedImages.length) {
      const firstSrcImage = requestedImages.find((image) => typeof image.src === 'string' && image.src);
