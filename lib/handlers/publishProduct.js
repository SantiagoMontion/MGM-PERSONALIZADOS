...
/** Ruta relativa al bucket outputs (p. ej. pdf-2026-03/foo.pdf) desde URL pública de Supabase. */
function parseOutputsStoragePathFromSupabasePublicUrl(rawUrl, bucket) {
  ...
  try {
    ...
    const needle = `/storage/v1/object/public/${b}/`;
    const idx = u.pathname.indexOf(needle);
    if (idx === -1) return '';
    let rel = decodeURIComponent(u.pathname.slice(idx + needle.length)).replace(/^\/+/, '');
