...
async function executeProductStatusUpdate({ productId, status = 'ACTIVE', maxAttempts = 2, metafields }) {
  const attempts = [];
  let lastError = null;
    ...
    if (!hasCreatedImages && productImagesInput.length && !restImageAttached) {
      ...
      if (firstSrcImage?.src) {
        try {
          ...
          const updateResp = await shopifyAdminGraphQL(PRODUCT_UPDATE_IMAGES_MUTATION, updateVariables);
