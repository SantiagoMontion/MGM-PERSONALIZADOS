...
const DEFAULT_MOCKUPS_BUCKET = process.env.SUPABASE_MOCKUPS_BUCKET || 'mockups';
...
async function blobLikeToBuffer(blob) {
  if (!blob) return null;
  ...
  if (typeof blob.buffer === 'object' && blob.buffer instanceof ArrayBuffer) {
    return Buffer.from(blob.buffer);
  }
  return null;
}

async function downloadSupabaseObject(bucket, objectKey) {
  if (!bucket || !objectKey) return null;
  try {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.storage.from(bucket).download(objectKey);
    if (error) {
      logger.warn('publish-product original_download_failed', {
        bucket,
        objectKey,
    ...
    const buffer = await blobLikeToBuffer(data);
    if (!buffer || !buffer.length) {
      logger.warn('publish-product original_download_empty', { bucket, objectKey });
      return null;
    }
    const contentType = typeof data?.type === 'string' && data.type ? data.type : null;
    return { buffer, contentType };
  } catch (err) {
    logger.error('publish-product original_download_exception', {
      bucket,
      objectKey,
...
async function fetchBufferFromUrl(url) {
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn('publish-product fetch_buffer_failed', {
        url,
    ...
    logger.warn('publish-product fetch_buffer_exception', {
      url,
      message: err?.message || err,
    });
    return null;
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const assets = body && typeof body.assets === 'object' && body.assets ? body.assets : {};
    const rid = pickFirstString(body, ['rid', 'diagId', 'diag_id']);
    const jobIdRaw = typeof body.jobId === 'string' && body.jobId.trim() ? body.jobId.trim() : '';
    const filenameInput = typeof body.filename === 'string' && body.filename ? body.filename : 'mockup.png';
    let filename = filenameInput;
    const warnings = [];
    let variantInventoryMode = 'not_tracked';
    let variantAvailableForSale = null;

    const mockupUrlCandidate =
      pickFirstString(body, ['mockupUrl', 'mockup_url'])
      || pickFirstString(assets, ['mockupUrl', 'mockup_url', 'previewUrl', 'preview_url']);
    const mockupBucketCandidate =
      pickFirstString(body, ['mockupBucket', 'mockup_bucket'])
      || pickFirstString(assets, ['mockupBucket', 'mockup_bucket']);
    const mockupObjectKeyCandidate =
      pickFirstString(body, ['mockupObjectKey', 'mockup_object_key'])
      || pickFirstString(assets, ['mockupObjectKey', 'mockup_object_key']);
    const mockupDataUrlRaw = typeof body.mockupDataUrl === 'string' ? body.mockupDataUrl.trim() : '';
    const mockupContentTypeCandidate =
      pickFirstString(body, ['mockupContentType', 'mockup_content_type'])
      || pickFirstString(assets, ['mockupContentType', 'mockup_content_type']);
    const mockupSizeBytes = toNumber(
      body.mockupSizeBytes ?? body.mockup_size_bytes ?? assets.mockupSizeBytes ?? assets.mockup_size_bytes,
    );

    let mockupBucket = mockupBucketCandidate || '';
    let mockupObjectKey = mockupObjectKeyCandidate || '';
    let mockupUrlUsed = mockupUrlCandidate || '';
    let mimeType = mockupContentTypeCandidate || '';
    let imageBuffer = null;

    if (mockupUrlCandidate) {
      try {
        const fetched = await fetch(mockupUrlCandidate);
        if (fetched.ok) {
          const arrayBuffer = await fetched.arrayBuffer();
          imageBuffer = Buffer.from(arrayBuffer);
          const headerMime = fetched.headers.get('content-type');
          if (typeof headerMime === 'string' && headerMime.trim()) {
            mimeType = headerMime.split(';')[0].trim();
          }
          mockupUrlUsed = fetched.url || mockupUrlCandidate;
          if ((!filename || filename === 'mockup.png') && fetched.url) {
            try {
              const inferred = new URL(fetched.url).pathname.split('/').pop();
              if (inferred) {
                filename = decodeURIComponent(inferred);
              }
            } catch {}
          }
        } else {
          const detail = await fetched.text().catch(() => '');
          logger.warn('publish-product mockup_download_failed', {
            status: fetched.status,
            url: mockupUrlCandidate,
            detail,
          });
        }
      } catch (err) {
        logger.warn('publish-product mockup_fetch_exception', {
          message: err?.message || err,
          url: mockupUrlCandidate,
        });
      }
    ...
    if ((!imageBuffer || !imageBuffer.length) && mockupDataUrlRaw) {
      const base64Payload = (mockupDataUrlRaw.split(',')[1] || '').trim();
      if (!base64Payload) {
        return res.status(400).json({ ok: false, reason: 'invalid_mockup_dataurl' });
      }
      const dataUrlMeta = parseDataUrlMeta(mockupDataUrlRaw);
      if (dataUrlMeta?.mimeType) {
        mimeType = dataUrlMeta.mimeType;
      }
      imageBuffer = Buffer.from(base64Payload, 'base64');
      mockupUrlUsed = mockupUrlUsed || '';
    }

    if (!imageBuffer || !imageBuffer.length) {
      return res.status(400).json({ ok: false, reason: 'missing_mockup_image_source' });
    }
    ...
    if ((!mockupBucket || !mockupObjectKey) && mockupUrlUsed) {
      const mockupParts = extractSupabaseObjectParts(mockupUrlUsed);
      if (mockupParts) {
        if (!mockupBucket) mockupBucket = mockupParts.bucket;
        if (!mockupObjectKey) mockupObjectKey = mockupParts.path;
      }
    }
    if (!mockupBucket) {
      mockupBucket = DEFAULT_MOCKUPS_BUCKET;
    }

    const originalUrlCandidate =
      pickFirstString(body, ['originalUrl', 'original_url', 'file_original_url', 'designUrl', 'design_url'])
      || pickFirstString(assets, ['originalUrl', 'original_url', 'file_original_url', 'designUrl', 'design_url']);
    const originalBucketCandidate =
      pickFirstString(body, ['originalBucket', 'original_bucket'])
      || pickFirstString(assets, ['originalBucket', 'original_bucket']);
    const originalObjectKeyCandidate =
      pickFirstString(body, ['originalObjectKey', 'original_object_key'])
      || pickFirstString(assets, ['originalObjectKey', 'original_object_key']);
    const originalContentType =
      pickFirstString(body, ['originalContentType', 'original_content_type'])
      || pickFirstString(assets, ['originalContentType', 'original_content_type']);
    const originalSizeBytes = toNumber(
      body.originalSizeBytes ?? body.original_size_bytes ?? assets.originalSizeBytes ?? assets.original_size_bytes,
    );

    let originalBucket = originalBucketCandidate || '';
    let originalObjectKey = originalObjectKeyCandidate || '';

    if ((!originalBucket || !originalObjectKey) && originalUrlCandidate) {
      const originalParts = extractSupabaseObjectParts(originalUrlCandidate);
      if (originalParts) {
        if (!originalBucket) originalBucket = originalParts.bucket;
        if (!originalObjectKey) originalObjectKey = originalParts.path;
      }
    }
    if (!originalBucket) {
      originalBucket = DEFAULT_UPLOADS_BUCKET;
    }
    ...
    const printDataUrl = typeof body.printDataUrl === 'string' ? body.printDataUrl.trim() : '';
    const printSourceUrl = typeof body.printSourceUrl === 'string' ? body.printSourceUrl.trim() : '';
    const requestedPrintDpi = Number(body.printDpi);
    const normalizedPrintDpi = Number.isFinite(requestedPrintDpi) && requestedPrintDpi > 0 ? requestedPrintDpi : null;
    const printBackgroundColorRaw = typeof body.printBackgroundColor === 'string' ? body.printBackgroundColor.trim() : '';
    const printBackgroundColor = printBackgroundColorRaw || '#ffffff';
    ...
    const designSlug = slugifyName(designNameRaw || title);
    ...
    let pdfAsset = null;
    const rawPrintDataUrl = printDataUrl;
    const rawPrintSourceUrl = printSourceUrl;
    const approxDensity = typeof approxDpi === 'number' && Number.isFinite(approxDpi) ? approxDpi : null;
    const density = normalizedPrintDpi || (approxDensity && approxDensity > 0 ? approxDensity : 300);
    const background = printBackgroundColor || '#ffffff';
    ...
    if (!PDF_ON_PURCHASE_ONLY) {
      let printSource = await downloadSupabaseObject(originalBucket, originalObjectKey);
      if ((!printSource || !printSource.buffer?.length) && originalUrlCandidate) {
        const fetched = await fetchBufferFromUrl(originalUrlCandidate);
        if (fetched?.buffer?.length) {
          printSource = { buffer: fetched.buffer, contentType: fetched.contentType || null };
        }
      }
      if ((!printSource || !printSource.buffer?.length) && rawPrintSourceUrl) {
        const fetched = await fetchBufferFromUrl(rawPrintSourceUrl);
        if (fetched?.buffer?.length) {
          printSource = { buffer: fetched.buffer, contentType: fetched.contentType || null };
        }
      }
      if ((!printSource || !printSource.buffer?.length) && rawPrintDataUrl && rawPrintDataUrl.startsWith('data:')) {
        const match = /^data:([^;,]+);base64,(.+)$/i.exec(rawPrintDataUrl);
        if (match) {
          const buffer = Buffer.from(match[2], 'base64');
          if (buffer.length) {
            printSource = { buffer, contentType: match[1] || null };
          }
        }
      }
      if ((!printSource || !printSource.buffer?.length) && imageBuffer?.length) {
        printSource = { buffer: imageBuffer, contentType: mimeType || null };
      }

      if (printSource?.buffer?.length) {
        try {
          ...
          try {
            ...
            if (uploadErr?.code === 'supabase_object_too_large') {
              ...
              err.size = uploadErr.size;
              err.limit = uploadErr.limit;
              throw err;
            }
            throw uploadErr;
          }
          ...
          try {
            ...
            const imageHash = computeImageHash(printSource.buffer);
            ...
            const jobIdForRecord = jobIdRaw || null;
          ...
          warnings.push({
            code: pdfError?.code || 'pdf_generation_failed',
            message: 'No se pudo generar el PDF de impresión.',
            ...(pdfError?.limit ? { limitBytes: pdfError.limit } : {}),
            ...(pdfError?.size ? { sizeBytes: pdfError.size } : {}),
          });
        }
      } else if (rawPrintDataUrl || rawPrintSourceUrl || originalUrlCandidate) {
        logger.warn('publish-product pdf_source_unavailable', {
          rid: rid || null,
          jobId: jobIdRaw || null,
          hasOriginalUrl: Boolean(originalUrlCandidate),
          hasOriginalObject: Boolean(originalObjectKey && originalBucket),
          hasPrintSourceUrl: Boolean(rawPrintSourceUrl),
          hasPrintDataUrl: Boolean(rawPrintDataUrl),
        });
      }
    } else if (PDF_ON_PURCHASE_ONLY && (rawPrintDataUrl || rawPrintSourceUrl || originalUrlCandidate)) {
      try {
        logger.info('publish-product pdf_deferred', {
          rid: rid || null,
          jobId: jobIdRaw || null,
        });
      } catch {}
    ...
    const normalizedRid = rid || null;
    const jobId = jobIdRaw || null;
    const mockupSizeEstimate = mockupSizeBytes ?? (imageBuffer ? imageBuffer.length : null);
    const publishDetails = {
      rid: normalizedRid,
      jobId,
      productId: meta?.productId || meta?.productAdminId || null,
      variantId: meta?.variantId || meta?.variantAdminId || null,
      productHandle: meta?.productHandle || null,
      productUrl: meta?.productUrl || productUrl || null,
      original: {
        url: originalUrlCandidate || null,
        bucket: originalBucket || null,
        objectKey: originalObjectKey || null,
        contentType: originalContentType || null,
        sizeBytes: originalSizeBytes ?? null,
      },
      mockup: {
        url: mockupUrlUsed || mockupUrlCandidate || null,
        bucket: mockupBucket || null,
        objectKey: mockupObjectKey || null,
        contentType: mimeType || mockupContentTypeCandidate || null,
        sizeBytes: mockupSizeEstimate,
      },
      print: {
        sourceUrl: rawPrintSourceUrl || null,
        hasDataUrl: Boolean(rawPrintDataUrl),
        widthCm,
        heightCm,
        approxDpi,
        requestedDpi: normalizedPrintDpi,
        backgroundColor: printBackgroundColor || '#ffffff',
      },
    ...
    try {
      ...
      if (jobId) {
        const jobUpdates = {
          shopify_product_id: meta?.productAdminId || meta?.productId || null,
          shopify_variant_id: meta?.variantAdminId || meta?.variantId || null,
          shopify_product_url: meta?.productUrl || productUrl || null,
          preview_url: mockupUrlUsed || mockupUrlCandidate || null,
        };
        const { error: jobUpdateError } = await supabase
          ...
          .eq('job_id', jobId);
        if (jobUpdateError) {
          logger.warn('publish-product job_update_failed', {
            rid: normalizedRid,
            jobId,
      ...
      if (normalizedRid || meta?.productId || meta?.productAdminId) {
        const referencePayload = {
          rid: normalizedRid,
          product_id: meta?.productId || meta?.productAdminId || null,
          shopify_product_id: meta?.productAdminId || meta?.productId || null,
          original_bucket: originalBucket || null,
          original_object_key: originalObjectKey || null,
          original_url: originalUrlCandidate || null,
          mockup_bucket: mockupBucket || null,
          mockup_object_key: mockupObjectKey || null,
          mockup_url: mockupUrlUsed || mockupUrlCandidate || null,
        };
        const { error: referenceError } = await supabase
          .from('publish_product_references')
          .upsert(referencePayload, { onConflict: 'rid', ignoreDuplicates: false });
        if (referenceError) {
          logger.warn('publish-product reference_store_failed', {
            rid: normalizedRid,
            productId: referencePayload.product_id,
      ...
      const eventPayload = {
        event_name: 'publish_product',
        rid: normalizedRid,
        design_slug: designSlug || null,
        job_id: jobId,
        details: publishDetails,
      };
      const { error: mappingError } = await supabase.from('events').insert(eventPayload);
      if (mappingError) {
        logger.warn('publish-product mapping_store_failed', {
          rid: normalizedRid,
          jobId,
      ...
      logger.warn('publish-product persistence_failed', {
        rid: normalizedRid,
        jobId,
    ...
    if (pdfAsset) {
      ...
      const previewUrl = pdfAsset.previewPath
        ? `/api/prints/preview?path=${encodeURIComponent(pdfAsset.previewPath)}`
        : null;
