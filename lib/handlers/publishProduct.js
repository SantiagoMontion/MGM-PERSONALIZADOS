...
import { detectImageMime } from '../_lib/imageToPdf.js';
import fetchOriginalAsset, { normalizeOriginalReference } from '../_lib/uploads.js';
...
import uploadMockup from './uploadMockup.js';

const DEFAULT_VENDOR = 'MgMGamers';
const OUTPUT_BUCKET = 'outputs';
const MOCKUP_UPLOAD_BUCKET = 'uploads';
...
const PUBLISH_REFERENCES_TABLE = 'publish_refs';

function withCorsJson(res, status, payload) {
...
function resolveMockup(p) {
  ...
  const cands = [
    ...
    S(p?.mockup_dataUrl),
    S(p?.mockup_dataURL),
    S(p?.assets?.mockup?.url),
    S(p?.assets?.mockup?.objectUrl),
    S(p?.assets?.mockup?.object_url),
    S(p?.assets?.mockup?.dataUrl),
    S(p?.assets?.mockup?.data_url),
  ];
  let val = cands.find((v) => v.length > 20);
  if (!val && typeof getMockupFromPayload === 'function') {
    const selection = getMockupFromPayload(p);
    if (selection?.type === 'url') {
      val = S(selection.value);
    } else if (selection?.type === 'dataurl') {
      val = S(selection.value);
    }
  }
  if (!val) return { kind: 'none' };
  if (val.startsWith('data:')) return { kind: 'dataurl', value: val };
  return { kind: 'url', value: val };
}

async function buildShopifyImages({ mockup, title, imageAlt }) {
  const normalizedTitle = typeof title === 'string' ? title.trim() : '';
  const normalizedAlt = typeof imageAlt === 'string' && imageAlt.trim()
    ? imageAlt.trim()
    : normalizedTitle || 'mockup';
  const payload = mockup && typeof mockup === 'object' ? mockup : null;
  if (!payload) return [];

  const makeResult = (entry, mode) => {
    const list = [entry];
    list.mode = mode;
    return list;
  };
  ...
  if (payload.kind === 'url') {
    const url = typeof payload.value === 'string' ? payload.value : '';
    if (!url) return [];
    if (/^https?:\/\//i.test(url) && url.includes('/storage/v1/object/public/')) {
      return makeResult({ src: url, alt: normalizedAlt }, 'src');
    }
    if (/^https?:\/\//i.test(url)) {
      try {
        const buf = await fetch(url)
          .then((r) => {
            if (!r.ok) {
              const err = new Error('mockup_fetch_failed');
              err.status = r.status;
              throw err;
            }
        ...
        return makeResult({ attachment: buf.toString('base64'), filename: 'mockup.jpg', alt: normalizedAlt }, 'attachment');
      } catch (err) {
        try {
          logger.warn('build_shopify_images_fetch_failed', {
  ...
  if (payload.kind === 'dataurl') {
    ...
    if (!base64) return [];
    return makeResult({ attachment: base64, filename: 'mockup.jpg', alt: normalizedAlt }, 'dataurl');
  }

  return [];
}

async function uploadMockupDataUrl({ rid, dataUrl, filename, diagId }) {
  ...
  if (!result?.ok || !result.mockupUrl) {
    const error = new Error(result?.error || 'mockup_upload_failed');
    error.code = result?.error || 'mockup_upload_failed';
    throw error;
  }
  return {
    publicUrl: result.mockupUrl,
    objectKey: result.mockupObjectKey || null,
  };
}

function getMockupFromPayload(payload) {
  ...
  const sources = [
    { type: 'url', value: payload.mockupUrl },
    { type: 'dataurl', value: payload.mockup_dataurl },
    { type: 'dataurl', value: payload.mockupDataUrl },
    { type: 'dataurl', value: payload.mockup_dataUrl },
    { type: 'dataurl', value: payload.mockupDataURL },
    { type: 'dataurl', value: payload.mockup_dataURL },
    { type: 'url', value: payload?.assets?.mockup?.url },
    { type: 'url', value: payload?.assets?.mockup?.objectUrl },
    { type: 'url', value: payload?.assets?.mockup?.object_url },
    { type: 'dataurl', value: payload?.assets?.mockup?.dataUrl },
    { type: 'dataurl', value: payload?.assets?.mockup?.data_url },
  ];

  for (const entry of sources) {
    if (!entry) continue;
    const candidate = normalize(entry.value);
    if (!candidate) continue;
    if (entry.type === 'url') {
      if (DATA_URL_PREFIX_REGEX.test(candidate)) continue;
      if (/^(blob:|file:)/i.test(candidate)) continue;
      return { type: 'url', value: candidate };
    }
    if (entry.type === 'dataurl') {
      if (!DATA_URL_PREFIX_REGEX.test(candidate)) continue;
      return { type: 'dataurl', value: candidate };
    }
  }
...
function respondMissingMockup({ res, body, diagId, originalAssetRef, detail, hasMockupCandidate }) {
  logPresentBodyFields({ body, status: 400, diagId, mockupUrl: null });
  const hasOriginal = Boolean(
    originalAssetRef?.originalObjectKey
      || originalAssetRef?.originalUrl
  );
  try {
    logger.warn('publish_product_validation_failed', {
      diagId: diagId || null,
      missing: ['mockup'],
      hasMockup: Boolean(hasMockupCandidate),
      hasOriginal,
    });
  } catch {}
  ...
  return withCorsJson(res, 400, payload);
}
...
function normalizeMockupPayload(body) {
  const empty = { originalUrl: '', previewUrl: '', imageUrl: '', dataUrl: '', mockupUrl: '' };
  ...
  const mockupUrlRaw = pickStringFromSources(sources, [
    'mockupUrl',
    'mockup_url',
  ]);
  ...
  return {
    ...
    mockupUrl: normalizeUrl(mockupUrlRaw),
  };
}

function extractOriginalAssetReference(body) {
  if (!body || typeof body !== 'object') {
    return { originalObjectKey: '', originalUrl: '', originalBucket: '' };
  }

  const assets = body?.assets && typeof body.assets === 'object' ? body.assets : null;
  const uploads = body?.upload && typeof body.upload === 'object' ? body.upload : null;
  const job = body?.job && typeof body.job === 'object' ? body.job : null;
  const sources = [assets, uploads, job, body];

  const originalObjectKey = pickStringFromSources(sources, [
    'originalObjectKey',
    'original_object_key',
    'objectKey',
    'object_key',
  ...
  const originalBucket = pickStringFromSources(sources, [
    'originalBucket',
    'original_bucket',
    'bucket',
  ]);

  const originalUrl = pickStringFromSources([
    assets,
    uploads,
    job,
    body,
  ...
  return normalizeOriginalReference({
    originalObjectKey,
    originalBucket,
    originalUrl,
  });
...
function extractMockupReference({ body, mockupPayload, coverImageSupabase, coverImageSource }) {
  const mockupSources = [];
  if (mockupPayload && typeof mockupPayload === 'object') {
    mockupSources.push(mockupPayload);
  }
  if (body?.mockup && typeof body.mockup === 'object') {
    mockupSources.push(body.mockup);
    if (body.mockup.assets && typeof body.mockup.assets === 'object') {
      mockupSources.push(body.mockup.assets);
    }
  }
  if (body?.assets && typeof body.assets === 'object') {
    mockupSources.push(body.assets);
  }
  mockupSources.push(body || {});

  const fallbackMockupUrl = pickStringFromSources(mockupSources, [
    'mockupUrl',
    'mockup_url',
    'previewUrl',
    'preview_url',
  ...
  const mockupObjectKey = pickStringFromSources(mockupSources, [
    'mockupObjectKey',
    'mockup_object_key',
    'objectKey',
    'object_key',
  ...
  const normalizedCoverSource = typeof coverImageSource === 'string' ? coverImageSource.trim() : '';
  const resolvedUrl = HTTP_URL_REGEX.test(normalizedCoverSource)
    ? normalizedCoverSource
    : typeof mockupPayload?.previewUrl === 'string' && mockupPayload.previewUrl.trim()
      ? mockupPayload.previewUrl.trim()
      : typeof mockupPayload?.imageUrl === 'string' && mockupPayload.imageUrl.trim()
        ? mockupPayload.imageUrl.trim()
        : fallbackMockupUrl;

  return {
    mockupUrl: resolvedUrl || '',
    mockupObjectKey: coverImageSupabase?.path || mockupObjectKey || '',
    mockupBucket: coverImageSupabase?.bucket || '',
  };
}

function extractRidFromRequest(req, body) {
  ...
  const headerNames = ['x-rid', 'x-mgm-rid', 'x-flow-rid'];
...
function collectPresentBodyKeys(body) {
  const keys = new Set();
  ...
  function walk(value) {
    ...
    for (const [key, val] of Object.entries(value)) {
      if (typeof key === 'string' && key) {
        const shouldInclude = !(typeof val === 'string' && !val.trim());
        if (shouldInclude) {
          keys.add(key);
        }
      }
...
function logPresentBodyFields({ body, status, diagId, mockupUrl }) {
  if (!body || typeof body !== 'object') return;
  try {
    const presentFields = collectPresentBodyKeys(body);
    const hasMockup = typeof mockupUrl === 'string' ? Boolean(mockupUrl.trim()) : false;
    logger.info('publish_product_present_fields', {
      status,
    ...
    try {
      logger.warn('publish_product_present_fields_error', {
...
async function persistPublishReference({
  rid,
  productId,
  productHandle,
  productUrl,
  ...
  try {
    ...
    const normalizedSize = (() => {
      if (!sizeMm || typeof sizeMm !== 'object') return null;
      const width = Number(sizeMm.w ?? sizeMm.width ?? sizeMm.widthMm ?? sizeMm.width_mm);
      const height = Number(sizeMm.h ?? sizeMm.height ?? sizeMm.heightMm ?? sizeMm.height_mm);
      if (!Number.isFinite(width) || width <= 0) return null;
      if (!Number.isFinite(height) || height <= 0) return null;
      return { w: width, h: height };
    })();
    const normalizedMargin = Number.isFinite(Number(marginMm)) ? Number(marginMm) : null;
    const payload = {
      rid,
      ...
      margin_mm: normalizedMargin,
      original_object_key: originalObjectKey || null,
    ...
    const { error } = await supabase
      .from(PUBLISH_REFERENCES_TABLE)
      .upsert(payload, { onConflict: 'rid', ignoreDuplicates: false });
    if (error) {
      logger.warn('publish_product_refs_upsert_failed', {
        rid,
        productId,
        message: error?.message || String(error),
    ...
    logger.warn('publish_product_refs_upsert_exception', {
      rid,
      productId,
      message: err?.message || String(err),
    });
...
async function createProductImageRest({ productId, image, maxAttempts = 2 }) {
  const attempts = [];
  let lastError = null;
  const legacyId = extractLegacyId(productId, 'Product');
  if (!legacyId) {
    throw new Error('product_image_rest_id_invalid');
  }
  const body = JSON.stringify({ image });

  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const resp = await shopifyAdmin(`products/${legacyId}/images.json`, { method: 'POST', body });
      const requestId = logRequestId('product_image_create_request', resp, {
        attempt: attempt + 1,
        productId: legacyId,
      });
      const json = await resp.json().catch(() => null);
      ...
      try {
        logger.warn('product_image_create_retry', {
          attempt: attempt + 1,
          status: resp.status,
          requestId: requestId || null,
          productId: legacyId,
      ...
      try {
        logger.warn('product_image_create_retry_error', {
          attempt: attempt + 1,
          message: err?.message || String(err),
          productId: legacyId,
        });
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const rid = extractRidFromRequest(req, body);
    const filename = typeof body.filename === 'string' && body.filename ? body.filename : 'mockup.jpg';
    const mockupResolution = resolveMockup(body);
    if (mockupResolution.kind === 'none') {
      const hasOriginalDirect = Boolean(
        (typeof body.originalObjectKey === 'string' && body.originalObjectKey.trim())
      ...
      try {
        logger.warn('publish_product_validation_failed', {
          ...
          missing: ['mockup'],
          hasMockup: false,
          hasOriginal: hasOriginalDirect,
        });
      } catch {}
      const payload = { ok: false, error: 'missing_mockup', missing: ['mockup'] };
      if (diagId) {
        payload.diagId = diagId;
      }
      logPresentBodyFields({ body, status: 400, diagId, mockupUrl: null });
      return withCorsJson(res, 400, payload);
    }
    ...
    const mockupSelection = getMockupFromPayload(body) || null;
    const originalAssetRef = extractOriginalAssetReference(body);
    const originalMimeFromBody = pickStringFromSources([
      body?.assets,
      body?.upload,
      body?.job,
      body,
    ...
    const warnings = [];
    const originalReferenceUrl = originalAssetRef.originalUrl || mockupPayload.originalUrl || '';
    let originalBuffer = null;
    let originalMime = originalMimeFromBody || null;
    let variantInventoryMode = 'not_tracked';
    let variantAvailableForSale = null;

    let mockupUrl = mockupResolution.kind === 'url' ? mockupResolution.value : '';
    let rawDataUrl = mockupResolution.kind === 'dataurl' ? mockupResolution.value : '';
    if (!mockupUrl && typeof mockupPayload.mockupUrl === 'string') {
      const trimmedMockup = mockupPayload.mockupUrl.trim();
      if (trimmedMockup) {
        mockupUrl = trimmedMockup;
      }
    }
    if (!rawDataUrl && typeof mockupPayload.dataUrl === 'string') {
      const trimmedData = mockupPayload.dataUrl.trim();
      if (trimmedData) {
        rawDataUrl = trimmedData;
      }
    }
    if (mockupSelection) {
      if (!mockupUrl && mockupSelection.type === 'url') {
        mockupUrl = mockupSelection.value;
      } else if (!rawDataUrl && mockupSelection.type === 'dataurl') {
        rawDataUrl = mockupSelection.value;
      }
    }
    ...
    let uploadedMockup = null;

    if (mockupResolution.kind === 'dataurl') {
      try {
        uploadedMockup = await uploadMockupDataUrl({
          rid,
          dataUrl: mockupResolution.value,
          filename,
          diagId,
        });
        mockupUrl = uploadedMockup.publicUrl || '';
        rawDataUrl = '';
      } catch (err) {
        stagedDataUrlError = err?.code || 'mockup_upload_failed';
        mockupUrl = '';
        rawDataUrl = '';
        try {
          logger.warn('publish_product_mockup_upload_exception', {
            diagId: diagId || null,
            rid: rid || null,
            message: err?.message || String(err),
            code: stagedDataUrlError,
          });
        } catch {}
      }
    }
    ...
    let hasMockupUrl = Boolean(mockupUrl);
    const hasMockupCandidate = Boolean(mockupUrl || rawDataUrl || previewUrl || imageUrl);
    const hasUsableMockupDataUrl = !hasMockupUrl && hasRawDataUrl
      ? DATA_URL_PREFIX_REGEX.test(rawDataUrl)
      : false;

    if (mockupUrl && !mockupPayload.mockupUrl) {
      mockupPayload.mockupUrl = mockupUrl;
    }

    if (!hasMockupUrl && !hasUsableMockupDataUrl) {
      return respondMissingMockup({
        res,
        body,
        diagId,
        originalAssetRef,
        hasMockupCandidate,
        detail: stagedDataUrlError ? { reason: stagedDataUrlError } : null,
      });
    }
    ...
    let mockupReference = { mockupUrl: '', mockupObjectKey: '', mockupBucket: '' };

    if (uploadedMockup && uploadedMockup.publicUrl) {
      coverImageSource = uploadedMockup.publicUrl;
      coverSourceUsed = 'dataUrl';
      coverImageMode = 'supabase';
      if (uploadedMockup.objectKey) {
        coverImageSupabase = {
          bucket: MOCKUP_UPLOAD_BUCKET,
          path: uploadedMockup.objectKey,
          signed: false,
          visibility: null,
        };
      }
      hasMockupUrl = Boolean(mockupUrl);
    }

    if (!uploadedMockup && hasUsableMockupDataUrl) {
      try {
        const uploadResult = await uploadMockup({
          rid: rid || null,
          mockupDataUrl: rawDataUrl,
          fileName: filename,
          diagId,
        });
        if (uploadResult?.ok && uploadResult.mockupUrl) {
          mockupUrl = uploadResult.mockupUrl;
          hasMockupUrl = true;
          coverImageSource = uploadResult.mockupUrl;
          coverSourceUsed = 'dataUrl';
          coverImageMode = 'supabase';
          if (uploadResult.mockupObjectKey) {
            coverImageSupabase = {
              bucket: MOCKUP_UPLOAD_BUCKET,
              path: uploadResult.mockupObjectKey,
              signed: false,
              visibility: null,
            };
          }
        } else {
          stagedDataUrlError = uploadResult?.error || 'invalid_mockup_dataurl';
          try {
            logger.warn('publish_product_mockup_upload_failed', {
              diagId: diagId || null,
              rid: rid || null,
    ...
    } else if (!hasMockupUrl && hasRawDataUrl) {
      stagedDataUrlError = 'invalid_mockup_dataurl';
    }

    if (!coverImageSource && mockupUrl && HTTP_URL_REGEX.test(mockupUrl)) {
      coverImageSource = mockupUrl;
      coverSourceUsed = 'mockupUrl';
      coverImageMode = 'src';
    }

    if (!coverImageSource) {
      const candidates = [
        { key: 'mockupUrl', value: mockupUrl },
    ...
    const hasAnyUrlCandidate = hasPreview || hasOriginal || hasImageUrl || hasMockupUrl;
    if (!coverImageSource) {
      const detail = stagedDataUrlError && !hasAnyUrlCandidate ? { reason: stagedDataUrlError } : null;
      return respondMissingMockup({
        res,
        body,
        diagId,
    ...
    const mockupSrc = coverImageSource;
    const mockupUsed = coverImageMode === 'attachment' ? 'dataurl' : 'url';

    try {
      logger.debug('publish_product_mockup_selected', {
        diagId: diagId || null,
        mockup_used: mockupUsed,
      });
    } catch {}
    ...
    try {
      logger.debug('publish_product_cover_source', {
        ...
        hasImageUrl,
        hasMockupUrl,
        hasDataUrl: hasRawDataUrl,
        hasUsableMockupDataUrl,
        resolvedUrl: coverImageSource || null,
        supabase: coverImageSupabase || null,
      });
    } catch {}

    mockupReference = extractMockupReference({
      body,
      mockupPayload,
      coverImageSupabase,
      coverImageSource,
    ...
    if (!mockupReference.mockupUrl && !mockupReference.mockupObjectKey) {
      return respondMissingMockup({
        res,
        body,
        diagId,
    ...
    const mockupForImages = (() => {
      if (mockupResolution.kind === 'dataurl' && mockupResolution.value) {
        return { kind: 'dataurl', value: mockupResolution.value };
      }
      if (mockupResolution.kind === 'url' && mockupResolution.value) {
        return { kind: 'url', value: mockupResolution.value };
      }
      if (rawDataUrl) {
        return { kind: 'dataurl', value: rawDataUrl };
      }
      if (mockupUrl) {
        return { kind: 'url', value: mockupUrl };
      }
      if (coverImageSource) {
        return DATA_URL_PREFIX_REGEX.test(coverImageSource)
          ? { kind: 'dataurl', value: coverImageSource }
          : { kind: 'url', value: coverImageSource };
      }
      return { kind: 'none' };
    })();
    ...
    const sizeMmPayload = body && typeof body.size_mm === 'object' && body.size_mm
      ? body.size_mm
      : body && typeof body.sizeMm === 'object' && body.sizeMm
        ? body.sizeMm
        : null;
    const widthMmRaw = toNumber(sizeMmPayload?.w ?? sizeMmPayload?.width ?? sizeMmPayload?.width_mm ?? sizeMmPayload?.widthMm);
    const heightMmRaw = toNumber(sizeMmPayload?.h ?? sizeMmPayload?.height ?? sizeMmPayload?.height_mm ?? sizeMmPayload?.heightMm);
    const widthCm = (() => {
      const direct = toNumber(body.widthCm ?? body.width_cm);
      if (direct != null) return direct;
      return widthMmRaw != null ? widthMmRaw / 10 : null;
    })();
    const heightCm = (() => {
      const direct = toNumber(body.heightCm ?? body.height_cm);
      if (direct != null) return direct;
      return heightMmRaw != null ? heightMmRaw / 10 : null;
    })();
    const hasCompleteSizeMm = Boolean(sizeMmPayload && widthMmRaw != null && heightMmRaw != null);
    const sizeMmForPersist = (() => {
      if (widthMmRaw != null && heightMmRaw != null) {
        return { w: widthMmRaw, h: heightMmRaw };
      }
      if (widthCm != null && heightCm != null) {
        return { w: widthCm * 10, h: heightCm * 10 };
      }
      return null;
    })();
    ...
    const designSlugFromBody = typeof body.design_slug === 'string' ? body.design_slug.trim() : '';
    if (!designSlugFromBody) {
      warnings.push({
        code: 'design_slug_missing',
        message: 'Falta design_slug en el payload. Se generó uno automáticamente.',
      });
    }
    if (!hasCompleteSizeMm) {
      warnings.push({
        code: 'size_missing',
        message: 'Faltan las medidas size_mm del producto.',
      });
    }
    if (!materialRaw) {
      warnings.push({
        code: 'material_missing',
        message: 'Falta el material del producto.',
      });
    }
    const designSlug = designSlugFromBody || slugifyName(designNameForPdf) || 'diseno';
    ...
    const shopifyImages = await buildShopifyImages({ mockup: mockupForImages, title, imageAlt });
    ...
    let productMediaInput = mockupSrc
      ? [{ mediaContentType: 'IMAGE', originalSource: mockupSrc, alt: imageAlt }]
      : [];
    ...
    const productImageMode = shopifyImages.length
      ? (typeof shopifyImages.mode === 'string' ? shopifyImages.mode : shopifyImages[0]?.src ? 'src' : 'attachment')
      : null;
    let productImageAttached = false;

    if (shopifyImages.length && meta.productId) {
      try {
        const { resp: imageResp, json: imageJson, requestId: imageRequestId } = await createProductImageRest({
          productId: meta.productId,
          image: shopifyImages[0],
        });
        const createdImage = imageJson?.image || null;
        productImageAttached = Boolean(imageResp?.ok && createdImage && (createdImage.id || createdImage.admin_graphql_api_id));
        if (!productImageAttached) {
          const warningPayload = {
            code: 'product_image_attach_failed',
            message: 'No se pudo asociar la imagen al producto recién creado.',
            requestId: imageRequestId || null,
            detail: imageResp?.ok ? imageJson : { status: imageResp?.status ?? null, body: imageJson },
          };
          warnings.push(warningPayload);
          try {
            logger.warn('product_image_attach_warning', warningPayload);
          } catch {}
        }
      } catch (err) {
        const warningPayload = {
          code: 'product_image_attach_exception',
          message: 'No se pudo asociar la imagen al producto recién creado.',
          detail: { message: err?.message || String(err) },
        };
        warnings.push(warningPayload);
        try {
          logger.warn('product_image_attach_exception', warningPayload);
        } catch {}
      }
    }
    ...
    if (productImageMode) {
      try {
        logger.info('publish_product_image_mode', {
          diagId: diagId || null,
          mode: productImageMode,
          hasImages: Boolean(productImageAttached),
        });
      } catch {}
    }
    ...
    const originalUrlForReference = originalAssetRef.originalUrl || originalUrl || '';
    const mockupUrlForReference = mockupReference.mockupUrl || coverImageSource || previewUrl || '';
    const productIdForReference = meta?.productId || meta?.productAdminId || null;
    const productHandleForReference = meta?.productHandle || null;
    const productUrlForReference = meta?.productUrl || null;

    await persistPublishReference({
      rid,
      productId: productIdForReference,
      productHandle: productHandleForReference,
      productUrl: productUrlForReference,
      designSlug,
      sizeMm: sizeMmForPersist,
      material: materialLabel || materialRaw || null,
      marginMm: 20,
      originalObjectKey: originalAssetRef.originalObjectKey,
      originalBucket: originalAssetRef.originalBucket,
      originalUrl: originalUrlForReference,
      originalMime,
      mockupUrl: mockupUrlForReference,
    });

    let pdfAsset = null;

    if (PDF_ON_PURCHASE_ONLY) {
      if (!originalAssetRef.originalObjectKey && !originalUrlForReference) {
        logger.warn('publish-product original_reference_missing', { diagId, rid });
        warnings.push({
          code: 'original_missing',
          message: 'No encontramos el archivo original para generar el PDF. Seguimos con la publicación.',
          detail: {
            originalObjectKey: null,
            originalUrl: originalUrlForReference || null,
          },
        });
      } else if (!originalAssetRef.originalObjectKey) {
        warnings.push({
          code: 'original_missing',
          message: 'El original no tiene object_key en Storage. Seguimos con la publicación.',
          detail: {
            originalObjectKey: null,
            originalUrl: originalUrlForReference || null,
          },
      ...
      const widthMm = widthValue != null ? widthValue * 10 : null;
      const heightMm = heightValue != null ? heightValue * 10 : null;
      const marginPerSideMm = 20;

      let shouldGeneratePdf = true;

      if (!originalAssetRef.originalObjectKey) {
        shouldGeneratePdf = false;
        logger.warn('publish-product original_not_found', {
          diagId,
          rid,
          originalObjectKey: null,
          originalUrl: originalUrlForReference || null,
      ...
      if (shouldGeneratePdf) {
        try {
          const originalAsset = await fetchOriginalAsset({
            supabase: null,
            originalObjectKey: originalAssetRef.originalObjectKey,
            originalBucket: originalAssetRef.originalBucket,
            originalUrl: originalUrlForReference,
          ...
          originalBuffer = originalAsset?.buffer || null;
        } catch (originalError) {
          if (originalError?.code === 'original_not_found') {
            logger.warn('publish-product original_not_found', {
              diagId,
              rid,
              originalObjectKey: originalAssetRef.originalObjectKey || null,
              originalUrl: originalUrlForReference || null,
            });
            warnings.push({
              code: 'original_missing',
            ...
            warnings.push({
              code: 'pdf_failed',
              message: 'No se pudo generar el PDF de impresión. Seguimos con la publicación.',
              detail: {
                reason: originalError?.code || null,
      ...
      if (shouldGeneratePdf && originalBuffer && originalBuffer.length) {
        try {
          const detectedOriginal = detectImageMime(originalBuffer);
          originalMime = detectedOriginal?.mime || originalMime;

          const pdfResult = await generatePrintPdf({
            widthCm: widthValue ?? undefined,
            heightCm: heightValue ?? undefined,
            sizeMm: widthMm != null && heightMm != null ? { w: widthMm, h: heightMm } : undefined,
          ...
          originalMime = pdfResult.info?.source?.mime || originalMime;
          const finalWidthCm = pdfResult.info?.pageWidthCm
            ?? (widthValue != null ? widthValue + (marginPerSideMm * 2) / 10 : null);
          const finalHeightCm = pdfResult.info?.pageHeightCm
            ?? (heightValue != null ? heightValue + (marginPerSideMm * 2) / 10 : null);
          const areaWidthCm = pdfResult.info?.area?.widthCm ?? widthValue;
          const areaHeightCm = pdfResult.info?.area?.heightCm ?? heightValue;

          const pdfSlug = designSlug;
          const pdfMetadata = {
            slug: pdfSlug,
            designName: designNameForPdf,
            widthCm: areaWidthCm ?? undefined,
            heightCm: areaHeightCm ?? undefined,
          ...
          try {
            ...
            const imageHash = computeImageHash(originalBuffer);
          ...
          if (pdfError?.code === 'original_not_found') {
            warnings.push({
              code: 'original_missing',
              message: 'No encontramos el archivo original para generar el PDF. Seguimos con la publicación.',
              detail: {
            ...
            warnings.push({
              ...
              message: 'No se pudo generar el PDF de impresión. Seguimos con la publicación.',
              detail: {
                reason: pdfError?.code || null,
                status: pdfError?.status || null,
              },
            });
          }
        }
    ...
    if (warnings.length) {
      try {
        logger.warn('publish_product_warnings', {
          diagId: diagId || null,
          warnings,
          warningCodes,
        });
      ...
      if (
        originalAssetRef.originalObjectKey
        || originalAssetRef.originalBucket
        || originalMime
        || originalUrlForReference
      ) {
        responsePayload.assets = {
          ...(responsePayload.assets || {}),
          original_object_key: originalAssetRef.originalObjectKey || null,
          original_bucket: originalAssetRef.originalBucket || null,
          original_url: originalUrlForReference || null,
          original_mime: originalMime || null,
        };
      }
      ...
      const previewPath = typeof pdfAsset.previewPath === 'string' && pdfAsset.previewPath
        ? pdfAsset.previewPath
        : '';
      const previewUrl = previewPath
        ? `/api/prints/preview?path=${encodeURIComponent(previewPath)}`
        : null;
      const assetsPayload = {
        ...(responsePayload.assets || {}),
      ...
      if (previewUrl) {
        assetsPayload.preview_url = previewUrl;
      }
      responsePayload.assets = assetsPayload;
      responsePayload.pdf = {
        path: pdfAsset.path,
        downloadUrl,
    ...
    logPresentBodyFields({ body, status: 200, diagId, mockupUrl });
