...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const privateMetafieldValue = isPrivate ? 'true' : 'false';
...

    {
      ...
      try {
        ...
        const metafieldUserErrors = rawMetafieldUserErrors
          .map((err) => {
            if (!err || typeof err !== 'object') return null;
            const message = typeof err.message === 'string' ? err.message.trim() : '';
            const field = sanitizeUserErrorField(err.field);
            const code = typeof err.code === 'string' ? err.code.trim() : '';
            if (!message && !field && !code) return null;
            return {
              ...(field ? { field } : {}),
              ...(message ? { message } : {}),
              ...(code ? { code } : {}),
        ...
        if (!metafieldResp.ok || metafieldErrors.length || !metafieldPayload) {
          const warnPayload = {
            requestId: metafieldRequestId || null,
            productId: productIdForVariants,
            value: privateMetafieldValue,
        ...
        } else if (!metafieldUserErrors.length) {
          try {
