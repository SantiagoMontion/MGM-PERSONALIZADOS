...
async function waitUrlOk(url, tries = 10, delayMs = 350) {
  ...
  if (url.startsWith('blob:')) {
    try {
      logger?.warn?.('mockup_head_skip_blob', { url });
    } catch {}
    return false;
  }
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const templateSuffix =
      materialResolvedRaw === 'Glasspad' || materialLabel === 'Glasspad'
        ? 'glasspads'
        : undefined;
    ...
    const fallbackMediaSource = !stagedImage?.originalSource && mockupSrcOnly && isHttpsUrl(mockupUrlRaw)
      ? mockupUrlRaw
      : null;
    ...
    const productImagesInput = requestedImages
      .map((image) => {
        ...
        const withAlt = image.altText ? image : { ...image, altText: imageAlt };
        if (withAlt?.src && !isHttpsUrl(withAlt.src)) {
          const { src, ...rest } = withAlt;
          return rest;
        }
        return withAlt;
      })
    ...
    try {
      console.log('[audit:shopify:create]', {
        ...
        templateSuffix,
        mockupSrcPresent: isHttpsUrl(body?.mockupUrl),
      });
    } catch (_) {
      // noop
    }
      ...
      try {
        ...
        try {
          console.log('[audit:shopify:image]', {
            ...
            title,
            priceToSet: priceValue,
            mockupSrcPresent: isHttpsUrl(mockupSrc),
          });
        } catch (_) {
          // noop
        }
        ...
        if (productIdForRest && adminToken && shopDomain) {
          ...
          if (mockupSrc && isHttpsUrl(mockupSrc)) {
          ...
          } else if (mockupSrc && !isHttpsUrl(mockupSrc)) {
            try {
              logger?.warn?.('product_image_skip_blob', { src: mockupSrc });
            } catch {}
          } else {
            try {
    ...
    if (!hasCreatedImages && productImagesInput.length && !restImageAttached) {
      const firstSrcImage = productImagesInput.find((image) => typeof image.src === 'string' && image.src);
      if (firstSrcImage?.src && isHttpsUrl(firstSrcImage.src)) {
        try {
      ...
      } else if (firstSrcImage?.src && !isHttpsUrl(firstSrcImage.src)) {
        try {
          logger?.warn?.('product_image_skip_blob', { src: firstSrcImage.src });
        } catch {}
      } else {
        try {
          logger?.warn?.('product_image_graphql_missing_src');
        } catch {}
      }
    }
