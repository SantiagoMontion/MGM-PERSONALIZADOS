...
function normalizeProductTypeForTemplateSuffix(productTypeRaw) {
  if (typeof productTypeRaw !== 'string') return '';
...
function resolveTemplateSuffixByProductType(productTypeRaw) {
  const normalized = normalizeProductTypeForTemplateSuffix(productTypeRaw);
  if (normalized === 'glasspad' || normalized.includes('glass')) return 'glasspads';
...
export async function publishProduct(req, res) {
  ...
  try {
    ...
    const productTypeForTemplateSuffix = String(productTypeRaw || body?.options?.productType || body?.productType || '');
    const productTypeForTemplateSuffixNormalized = normalizeProductTypeForTemplateSuffix(productTypeForTemplateSuffix);
    const materialForTemplateSuffix = String(body?.material || body?.materialResolved || body?.options?.material || '').trim();
    const mustForceGlasspadTemplate = productTypeForTemplateSuffixNormalized === 'mousepad'
      && /glass/i.test(materialForTemplateSuffix);
    /** Serie Ultra: plantilla de tema Shopify dedicada (resto de materiales sin cambios). */
    const mustForceUltraPoronTemplate = materialLabel === 'Ultra';
    const templateSuffix = mustForceGlasspadTemplate
      ? 'glasspads'
      : mustForceUltraPoronTemplate
        ? 'mousepads-poron'
        : resolveTemplateSuffixByProductType(productTypeForTemplateSuffix);

    try {
      logger.info('shopify_template_suffix_resolved', {
        productTypeRaw: productTypeForTemplateSuffix || null,
        productTypeNormalized: productTypeForTemplateSuffixNormalized || null,
        material: materialForTemplateSuffix || null,
        forcedGlasspad: mustForceGlasspadTemplate,
        forcedUltraPoron: mustForceUltraPoronTemplate,
    ...
    try {
      ...
      if (rawPrintDataUrl || rawPrintSourceUrl) {
        ...
        if (existingPdfPath) {
          ...
          } catch (printsErrExisting) {
            logger.error('publish-product prints_upsert_from_pdf_url', { error: printsErrExisting?.message || printsErrExisting });
          }
