...
import { applyEquilibriumListPrice } from '../pricing/equilibrium.js';
...
function finalizeServerListPricing(netBasePrice) {
...
function calculateTransferPrice({ material, productTypeKey, widthCm, heightCm }) {
  ...
  if (mode === 'Lampara') {
    return finalizeServerListPricing(LAMP_TRANSFER_PRICE);
  }
  if (mode === 'Glasspad') {
    ...
    return finalizeServerListPricing(transferPriceRounded);
  }
  if (mode === 'Ultra') {
  ...
  if (!rollo) {
    return finalizeServerListPricing(0);
  }
