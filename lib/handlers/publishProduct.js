...
async function persistPublishReference({ rid, productId, originalObjectKey, originalUrl, mockupUrl }) {
  ...
  try {
    ...
    try {
      if (!PDF_ON_PURCHASE_ONLY) {
        ...
        const marginPerSideMm = marginPerSideCm * 10;
        const targetWidthMm = widthMm != null ? widthMm + marginPerSideMm * 2 : null;
        const targetHeightMm = heightMm != null ? heightMm + marginPerSideMm * 2 : null;
        ...
        const { buffer: originalBuffer } = await fetchOriginalAsset({
