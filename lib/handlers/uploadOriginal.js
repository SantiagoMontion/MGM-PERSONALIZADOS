...
function getMaxImageMbForBackend() {
  const raw = process.env.VITE_MAX_IMAGE_MB ?? process.env.MAX_IMAGE_MB;
  const n = Number(raw);
  return Number.isFinite(n) && n > 0 ? n : 30;
}

// Mismo límite que el front (`VITE_MAX_IMAGE_MB`, default 30MB).
const MAX_IMAGE_MB = getMaxImageMbForBackend();
const MAX_FILE_SIZE_BYTES = MAX_IMAGE_MB * 1024 * 1024;
const MAX_JSON_BODY_BYTES = Math.ceil((MAX_FILE_SIZE_BYTES * 4) / 3) + 1024 * 1024;
...
function parseDataUrl(value) {
  ...
  const estimatedSize = estimateBase64DecodedSize(b64);
  if (!estimatedSize || estimatedSize > MAX_FILE_SIZE_BYTES) return null;
  try {
    const buffer = Buffer.from(b64, 'base64');
    if (!buffer.length || buffer.length > MAX_FILE_SIZE_BYTES) return null;
...
function parseBase64(value, fallbackContentType) {
  ...
  const estimatedSize = estimateBase64DecodedSize(trimmed);
  if (!estimatedSize || estimatedSize > MAX_FILE_SIZE_BYTES) return null;
  try {
...
export default async function uploadOriginal(req, res) {
  ...
  if (declaredSize && declaredSize > MAX_FILE_SIZE_BYTES) {
    return res.status(413).json({
      ok: false,
      diag_id: diagId,
