...
export default async function uploadOriginal(req, res) {
  ...
  if (!Object.keys(payload).length) {
    try {
      ...
      console.error('upload-original read_body', { diagId, error: err?.message || err });
  ...
  if (declaredSize && declaredSize > 0 && size !== declaredSize) {
    console.warn('upload-original size_mismatch', {
      diagId,
      declaredSize,
      actualSize: size,
    });
  ...
  console.info('upload-original start', {
    diagId,
  ...
  try {
    ...
    if (error) {
      console.error('upload-original supabase', {
        diagId,
        bucketName: UPLOAD_BUCKET,
        path: objectKey,
        size,
    ...
    try {
      ...
      console.warn('upload-original signed_url', { diagId, error: err?.message || err });
    }
    ...
    console.error('upload-original exception', {
      diagId,
      bucketName: UPLOAD_BUCKET,
      path: objectKey,
