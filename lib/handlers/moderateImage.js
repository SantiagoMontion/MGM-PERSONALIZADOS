...
export async function evaluateImage(buffer, filename, designName = '', options = {}) {
  ...
  const meetsCoverage = (
    skinPercent >= 0.45 &&
    centerSkinPercent >= 0.35 &&
    largestBlob >= 0.3
  );

  const strongCenter = centerSkinPercent >= 0.6;
  const strongBlob = largestBlob >= 0.5 || largestBlobBoxCoverage >= 0.38;
  const highSkinCoverage = skinPercent >= 0.55;
  const notCartoon =
    cartoonConfidence < 0.75 ||
    paletteRatio >= 0.015 ||
    edgeRatio <= 0.03 ||
    toneVariance <= 0.18;
