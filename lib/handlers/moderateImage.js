...
async function detectNazi(buffer) {
  ...
  let inCircleCount = 0;
  let inRingCount = 0;
  // Disco central relativo al lado corto (bandera ancha 90×40).
  ...
  for (let p = 0, idx = 0; p < total; p++, idx += i2.channels) {
    ...
    const G = d2[idx + 1];
    const B = d2[idx + 2];
    const isRed = R > 180 && G < 110 && B < 110 && R > G + 40 && R > B + 40;
  ...
  const redScore = clamp((redRatio - 0.35) / 0.35);
  const whiteScore = clamp((whiteCircleRatio - 0.25) / 0.45);
  const blackScore = clamp((blackInDiskRatio - 0.03) / 0.12);
  let paletteSignal = clamp(redScore * 0.5 + whiteScore * 0.3 + blackScore * 0.2);

  // Bandera nazi a full-bleed (pads anchos estirados): rojo dominante + disco claro + negro en centro.
  const classicFlagHeuristic = (
    redRatio >= 0.4
    && whiteCircleRatio >= 0.25
    && blackInDiskRatio >= 0.025
  );
  ...
  return {
    ...
    palette: { redRatio, whiteCircleRatio, blackInDiskRatio, classicFlagHeuristic },
    likelyManji,
  };
}
...
export async function evaluateImage(buffer, filename, designName = '', options = {}) {
  ...
  debug.scores.nazi_min_dist = Number.isFinite(minDist) ? minDist : null;
  ...
  const paletteStrongAlone = paletteSignal >= NAZI_PALETTE_ALONE_THRESHOLD;

  let naziScore = 0;
  if (shapeStrongAlone) {
    // Esvástica clara y dominante: un solo match de forma alcanza.
    naziScore = shapeSignal;
  } else if (paletteStrongAlone) {
    // Bandera nazi a pantalla completa (rojo + círculo blanco), típica en pads anchos.
  ...
  debug.scores.nazi_shape_alone = shapeStrongAlone ? 1 : 0;
  debug.scores.nazi_palette_alone = paletteStrongAlone ? 1 : 0;
