...
async function detectNazi(buffer) {
  ...
  const samplePaletteAt = (cx, cy, radius) => {
    ...
    // Agujero claro: el centro es mucho menos rojo que el campo (disco blanco mezclado cuenta).
    const redHole = (
      redRatio >= 0.35
      && centerRedRatio < Math.min(0.4, redRatio * 0.55)
      && (brightCircleRatio >= 0.12 || whiteCircleRatio >= 0.12 || centerRedRatio < 0.25)
    );
    const classicFlagHeuristic = (
      (redRatio >= 0.38 && (whiteCircleRatio >= 0.18 || brightCircleRatio >= 0.18) && blackInDiskRatio >= 0.02)
      || (redHole && blackInDiskRatio >= 0.04)
    );
    return {
      redRatio,
      whiteCircleRatio,
  ...
  const blackScore = clamp((blackInDiskRatio - 0.02) / 0.12);
