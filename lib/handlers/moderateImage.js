...
async function detectSkin(buffer) {
  ...
  // Textura local en tonos “piel”: pelaje/tela tiene gradientes altos; piel humana lisa, bajos.
  // Evita falsos positivos de mascotas (perros, gatos, etc.).
  let textureAccum = 0;
  ...
  for (let y = 0; y < h - step; y += step) {
    for (let x = 0; x < w - step; x += step) {
      ...
      const pR = p + step;
      const pD = p + step * w;
      // Solo pares interior-piel (bordes del blob inflan la métrica).
      const hasR = x + step < w && mask[pR];
      const hasD = y + step < h && mask[pD];
      if (!hasR && !hasD) continue;
      const idx = p * info.channels;
      const Y0 = 0.299 * data[idx] + 0.587 * data[idx + 1] + 0.114 * data[idx + 2];
      if (hasR) {
        const idxR = pR * info.channels;
        const YR = 0.299 * data[idxR] + 0.587 * data[idxR + 1] + 0.114 * data[idxR + 2];
        textureAccum += Math.abs(Y0 - YR);
        textureSamples += 1;
      }
      if (hasD) {
        const idxD = pD * info.channels;
        const YD = 0.299 * data[idxD] + 0.587 * data[idxD + 1] + 0.114 * data[idxD + 2];
        textureAccum += Math.abs(Y0 - YD);
        textureSamples += 1;
      }
    }
  }
  // Normalizado ~0–1. Empírico (resize 256): piel lisa ~0.05; perros reales ~0.16–0.22; pelaje sintético alto ~0.25+.
  const skinTexture = textureSamples ? Math.min(1, (textureAccum / textureSamples) / 55) : 0;
  const furLikelihood = clamp((skinTexture - 0.12) / 0.06);
...
function computeNudityConfidence({
  ...
  // Pelaje/tela: no contar como desnudez aunque el tono entre en el rango de piel.
  const furPenalty = clamp(1 - furLikelihood * 1.35);
  ...
  combined *= variancePenalty * furPenalty;
...
export async function evaluateImage(buffer, filename, designName = '', options = {}) {
  ...
  const furLikelihood = skin?.furLikelihood ?? 0;
  const skinTexture = skin?.skinTexture ?? 0;
  /** Mascotas / pelaje: tonos “piel” con textura alta no son desnudez humana. */
  const likelyAnimalFur = furLikelihood >= 0.45 && skinPercent >= 0.12;
  ...
  const isRealPerson = personScore >= PERSON_SCORE_THRESHOLD
    && realProbability >= REAL_PERSON_THRESHOLD
    && !likelyAnimalFur;
  ...
  debug.scores.skin_texture = skinTexture;
  debug.scores.fur_likelihood = furLikelihood;
  debug.scores.likely_animal_fur = likelyAnimalFur ? 1 : 0;
  ...
  // Areola: exigir piel relativamente lisa. Solo “piel” en el centro dispara falsos positivos en mascotas.
  const areolaScore = (furLikelihood >= 0.35 || likelyAnimalFur)
    ? 0
    : clamp((centerSkinPercent - 0.55) / 0.25) * clamp(1 - furLikelihood * 2);
