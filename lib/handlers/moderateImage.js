...
export async function evaluateImage(buffer, filename, designName = '', options = {}) {
  ...
  if (!blockReasons.size && !metaGate.blocked) {
    ...
    const shouldBlockByConfidence =
      !shouldBlockStrong &&
      nudityConfidence >= 0.6 && // Increased from 0.55 to reduce false positives
      meetsCoverage &&
      notCartoon &&
      centerSkinPercent >= 0.25 &&
      largestBlob >= 0.25;

    // Enhanced landscape filter: High edge ratio, moderate cartoon confidence, and low center ratio
    const isLikelyLandscape = edgeRatio > 0.2 && cartoonConfidence >= 0.3 && largestBlobCenterRatio < 0.25;
    if ((shouldBlockStrong || shouldBlockByConfidence) && !isLikelyLandscape) {
      const baseConfidence = shouldBlockStrong ? 0.78 : 0.7;
      const confidence = clamp(baseConfidence + (nudityConfidence - 0.6) * 0.5);
