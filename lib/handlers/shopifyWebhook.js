    ...
    if (!Array.isArray(existing) || existing.length === 0) {
      ...
      try {
        ...
        if (trackError && trackError.code !== '23505') {
          console.warn('shopify-webhook track_events insert warning', {
            diagId,
            topic,
            orderId: orderIdString,
            ridFound: Boolean(rid),
            error: trackError?.message || trackError,
          });
        }
      } catch (trackErr) {
        console.warn('shopify-webhook track_events unexpected warning', {
          diagId,
          topic,
          orderId: orderIdString,
          ridFound: Boolean(rid),
