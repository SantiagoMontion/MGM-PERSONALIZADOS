...
async function processOrderPaidLossless({
  ...
  const publishUrl = safeTrim(publishRef.original_url);

  if (!publishObjectKey && !publishUrl) {
    logger.warn('shopify-webhook paid_original_reference_missing', {
      diagId,
      rid: normalizedRid,
      orderId,
    });
  ...
  try {
    originalAsset = await fetchOriginalAsset({
      ...
      originalObjectKey: publishObjectKey,
      originalBucket: UPLOADS_BUCKET,
      originalUrl: publishUrl,
      rid: normalizedRid,
      diagId,
    });
  } catch (err) {
    if (err?.code === 'original_not_found') {
      logger.warn('shopify-webhook paid_original_not_found', {
        ...
        rid: normalizedRid,
        orderId,
        objectKey: publishObjectKey || null,
        url: publishUrl || null,
      });
    } else {
  ...
  if (!isPdf) {
    ...
    try {
      ...
      if (err?.code === 'suspicious_output') {
        logger.warn('shopify-webhook paid_pdf_suspicious_output', {
          diagId,
          rid: normalizedRid,
          orderId,
        });
