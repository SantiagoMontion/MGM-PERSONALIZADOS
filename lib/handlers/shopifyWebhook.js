...
async function getRawRequestBody(req, { limit = RAW_BODY_LIMIT } = {}) {
  if (req && typeof req.text === 'function') {
    ...
    const rawBuffer = Buffer.from(typeof rawText === 'string' ? rawText : '', 'utf8');
    if (rawBuffer.length > limit) {
  ...
  if (req.body && typeof req.body === 'object') {
    // Fallback defensivo: evita explotar cuando algún middleware parseó antes.
    // La verificación de firma podrá fallar si el raw original no coincide.
    const reparsed = Buffer.from(JSON.stringify(req.body), 'utf8');
    if (reparsed.length > limit) {
      const err = new Error('Request entity too large');
      err.code = 'payload_too_large';
      throw err;
    }
...
export default async function shopifyWebhook(req, res) {
  ...
  const topic = String(getHeaderValue(req, 'x-shopify-topic') || '').trim();
  ...
  if (topic.startsWith('orders/')) {
    ...
    if (!order) {
      logger.warn('shopify-webhook invalid_order_payload', { diagId, topic });
    } else {
      purchaseResult = await processPaidShopifyOrder(order, { topic, diagId });
      if (purchaseResult?.recorded) {
        logger.info('shopify-webhook purchase_tracked', {
          diagId,
          topic,
          orderId: purchaseResult.orderId,
          rid: purchaseResult.rid,
        });
      } else if (purchaseResult && !purchaseResult.skipped && purchaseResult.ok === false) {
        logger.warn('shopify-webhook purchase_tracking_failed', {
          diagId,
          topic,
          reason: purchaseResult.reason || null,
  ...
  res.status(200).json({
    ok: true,
    ...(purchaseResult?.recorded ? { purchase_tracked: true } : {}),
  });
}
