...
function logEvent(event, payload, level = 'debug') {
  try {
    const logFn = level === 'warn' ? logger.warn : logger.debug;
    logFn(`[cart-link] ${event}`, payload);
  } catch {}
}
  ...
  try {
    ...
    try {
      storefrontAttempt = await createStorefrontCart({
        ...
        lineAttributes: normalizedAttributes,
        note,
      });
    } catch (err) {
      logEvent(
    ...
    logger.error('[cart-link] unhandled_error', err);
