...
const CART_ORIGIN = (process.env.SHOPIFY_CART_ORIGIN || 'https://notmid.ar').replace(/\/+$/, '');
...
function respond(res, status, payload, logLabel = 'cart_start_response') {
  ...
  try {
    logger.debug(logLabel, { status, contentType, body });
  } catch {}
}

export default async function cartStart(req, res) {
  try {
    logger.debug('cart_start_request', { method: req.method, path: req.url || '' });
  } catch {}
  ...
  if (!permalink) {
    try {
      logger.warn('cart_start_permalink_missing', { variantGid, variantNumericId });
    } catch {}
    return respond(res, 500, { ok: false, error: 'permalink_unavailable' });
  }

  try {
    logger.debug('cart_start_url_returned', {
      url: permalink,
      variantGid,
      variantNumericId,
