...
/** Precio neto PRO = precio de lista vigente × 0,76 (redondeo hacia abajo a $50, como Shopify). */
export function applyProSeriesNetFromListPrice(listPrice) {
  const list = Number(listPrice) || 0;
  if (list <= 0) return 0;
  return roundDownToNearestFifty(list * PRO_SERIES_NET_LIST_FACTOR);
}

export function resolveCustomerPriceFromNetBase(netBasePrice, { isProSeries = false } = {}) {
  // PRO: lista con redondeo matemático (como Shopify) y luego ×0,76 hacia abajo.
