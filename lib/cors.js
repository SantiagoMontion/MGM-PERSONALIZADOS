...
const STATIC_ALLOWED = [
  ...
  'https://notmid.ar',
  'https://www.notmid.ar',
  'https://tu-mousepad-personalizado.notmid.ar',
  'https://personalizados.notmid.ar',
  'https://www.personalizados.notmid.ar',
  'https://mgm-api.vercel.app',
  'https://mgm-app.vercel.app',
];
const STATIC_SUFFIXES = ['.vercel.app', '.notmid.ar'];
...
export function applyCorsHeaders(req, res, decision) {
  ...
  res.setHeader('Access-Control-Allow-Methods', resolved.allowMethods || DEFAULT_ALLOW_METHODS);
...
export function ensureCors(req, res, allowList) {
  ...
  if (!originHeader && url.startsWith('/api/shopify-webhook')) {
    const allowHeaders = '*';
