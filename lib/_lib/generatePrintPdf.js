...
export async function generatePrintPdf({
  ...
  let metadata;
  try {
    metadata = await sharp(imageBuffer, { failOnError: false }).metadata();
  } catch (err) {
  ...
  if (embedKind === 'jpeg') {
    embedFormat = 'jpeg';
  } else if (embedKind === 'png') {
    embedFormat = 'png';
  } else {
    convertedFrom = metadata?.format || 'unknown';
    embedFormat = 'png';
    embedBuffer = await sharp(imageBuffer, { failOnError: false }).png({ compressionLevel: 0, progressive: false, adaptiveFiltering: false }).toBuffer();
  }
  ...
  const drawX = marginPt + (areaWidthPt - drawWidth) / 2;
  const drawY = marginPt + (areaHeightPt - drawHeight) / 2;
  ...
  const uint8 = await pdfDoc.save({ useObjectStreams: false, addDefaultPage: false });
