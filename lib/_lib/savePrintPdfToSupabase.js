...
export async function savePrintPdfToSupabase(buffer, filename, metadata = {}) {
  ...
  if (size > maxObjectBytes) {
    ...
    error.size = size;
    console.error('pdf_upload_too_large', { diagId, bucket: OUTPUT_BUCKET, path, sizeBytes: size, limit: maxObjectBytes });
    throw error;
  }
...
export async function savePrintPreviewToSupabase(buffer, filename, metadata = {}) {
  ...
  try {
    ...
    console.error('preview_save_env_error', { diagId, message: err?.message || err });
    const error = new Error('Faltan credenciales de Supabase');
    error.code = 'supabase_credentials_missing';
    error.cause = err;
    throw error;
  ...
  if (size > maxObjectBytes) {
    ...
    error.limit = maxObjectBytes;
    error.size = size;
    console.error('preview_upload_too_large', { diagId, bucket: OUTPUT_BUCKET, path, sizeBytes: size, limit: maxObjectBytes });
    throw error;
  }
  console.info('preview_upload_start', { diagId, bucket: OUTPUT_BUCKET, path, sizeBytes: size });
  ...
  if (uploadError) {
    console.error('preview_upload_failure', {
      diagId,
      bucket: OUTPUT_BUCKET,
      path,
      sizeBytes: size,
  ...
  console.info('preview_upload_ok', { diagId, bucket: OUTPUT_BUCKET, path, sizeBytes: size });
