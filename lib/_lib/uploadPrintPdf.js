...
function resolveUploadSignedUrlTtl() {
  ...
  try {
    logger.warn('prints_upload_ttl_invalid', { raw });
  } catch (err) {
    // noop
  }
...
export async function uploadPrintPdf({ buffer, filename, metadata = {}, diagId }) {
  ...
  if (originalName && /\.(png|jpe?g)$/i.test(originalName)) {
    logger.warn('png_upload_blocked', {
      diagId: localDiag,
  ...
  try {
    ...
    logger.error('pdf_upload_env_error', { diagId: localDiag, message: err?.message || err });
  ...
  const details = buildPrintStorageDetails({
    ...
    straightEdges: metadata.straightEdges ?? metadata.straight_edges,
  ...
  if (size > MAX_PDF_BYTES) {
    logger.error('pdf_upload_too_large', { diagId: localDiag, path, size });
  ...
  if (uploadError) {
    logger.info('pdf_upload_duration', {
      diagId: localDiag,
      bucket: OUTPUT_BUCKET,
      path,
      stage: 'upload',
  ...
  const signResult = await Promise.race([
    ...
    new Promise((resolve) => {
      setTimeout(() => resolve({ data: null, error: new Error('supabase_signed_url_timeout') }), SIGNED_URL_TIMEOUT_MS);
    }),
  ]);
  const signedData = signResult?.data || null;
  const signedError = signResult?.error || null;
