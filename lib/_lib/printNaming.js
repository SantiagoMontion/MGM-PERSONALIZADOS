...
export function buildPrintFilenameFromMetadata({
  ...
  const rectoSegment = (straightEdges === true || straight_edges === true) ? 'recto' : null;
  const idSegment = pickIdSegment({ jobId, jobKey, fallbackFilename });
  const ext = String(extension || 'pdf').replace(/\.+$/, '').toLowerCase() || 'pdf';

  if (hasSize) {
    const parts = [slugSegment, `${widthSegment}x${heightSegment}`, materialSegment];
    if (rectoSegment) parts.push(rectoSegment);
