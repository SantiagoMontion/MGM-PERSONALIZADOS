...
import { insertTrackEventDeduped } from './trackEventInsert.js';
...
export function extractOrderTrackingFields(order) {
  ...
  const jobId = readAttributeValue(noteAttributes, ['job_id', '_job_id', 'jobid'])
    || lineProps.job_id
    || lineProps._job_id
    || lineProps.jobid
    || null;

  const material = readAttributeValue(noteAttributes, ['material', '_material'])
    || lineProps.material
    || lineProps._material
    || null;

  const measurementRaw = readAttributeValue(noteAttributes, ['measurement_cm', '_measurement_cm', 'measurement'])
    || lineProps.measurement_cm
    || lineProps._measurement_cm
  ...
  const designSlug = readAttributeValue(noteAttributes, ['design_slug', '_design_slug', 'designslug'])
    || lineProps.design_slug
    || lineProps._design_slug
...
export async function processPaidShopifyOrder(order, { topic, diagId } = {}) {
  ...
  try {
    const status = await insertTrackEventDeduped(supabase, insertPayload);
    if (status === 'duplicate') {
