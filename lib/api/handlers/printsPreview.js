...
export default async function printsPreviewHandler(req, res) {
  ...
  if (!rawPath) {
    sendJson(res, 400, { ok: false, reason: 'missing_path', message: 'Falta el parámetro "path".' });
    return;
  }
  if (!isValidPreviewPath(rawPath)) {
    sendJson(res, 400, { ok: false, reason: 'invalid_path', message: 'Formato de path inválido.' });
    return;
  }
  ...
  try {
    ...
    sendJson(res, 502, { ok: false, reason: 'supabase_init_failed', message });
    return;
  }
  ...
  if (!isPdf) {
    ...
    const attemptDownload = async (path) => {
      logger.info(`Buscando archivo en: ${path}`);
    ...
    if (!buffer) {
      ...
      for (const ext of candidates) {
        ...
        if (buffer === EMPTY_BUFFER) {
          sendJson(res, 500, { ok: false, reason: 'empty_file', message: 'El archivo descargado está vacío.' });
          return;
        }
      }
    ...
    if (!buffer) {
      sendJson(res, 404, { ok: false, reason: 'preview_not_found', message: 'No se encontró el archivo solicitado.' });
      return;
    }
  ...
  const previewBasePaths = (() => {
    ...
    const dash = withoutExt.match(/^pdf-(\d{4})-(\d{2})\/(.+)$/i);
    if (dash) {
      const safeBase = sanitizePdfFilename(`${dash[3]}.pdf`).replace(/\.pdf$/i, '');
      return [`preview/${dash[1]}/${dash[2]}/${safeBase}`];
    }
    const parts = withoutExt.split('/');
    if (parts.length <= 1) return [`preview/${withoutExt}`];
    const legacyMockups = withoutExt.replace(/^pdf-/i, 'mockups-');
    const primary = `preview/${parts.slice(1).join('/')}`;
    return legacyMockups !== withoutExt ? [primary, legacyMockups] : [primary];
  })();

  const previewCandidates = [];
  for (const base of previewBasePaths) {
    for (const ext of ['.jpg', '.jpeg', '.png', '.webp']) {
      previewCandidates.push(`${base}${ext}`);
    }
  }

  try {
    for (const candidate of previewCandidates) {
      logger.info(`Buscando archivo en: ${candidate}`);
      ...
      if (!previewError && previewData) {
        ...
        logger.debug('[prints-preview] preview_unavailable', { path: candidate, message: previewError.message });
      }
    }
  } catch (err) {
    logger.warn('[prints-preview] preview_fetch_failed', { paths: previewBasePaths, message: err?.message || err });
  }

  res.setHeader('Cache-Control', 'no-store');
  sendJson(res, 404, { ok: false, reason: 'preview_unavailable', message: 'No hay una vista previa disponible.' });
}
