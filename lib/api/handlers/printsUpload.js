...
function extractOriginalReference(payload) {
  const sources = [payload, payload?.assets, payload?.upload, payload?.job];
  ...
  return normalizeOriginalReference({ originalObjectKey, originalBucket, originalUrl });
}
...
export async function uploadPrintHandler(req, res) {
  ...
  const originalRef = extractOriginalReference(payload);
  if (!originalRef.originalObjectKey && !originalRef.originalUrl) {
    return res.status(400).json({ ok: false, diagId, requestId, reason: 'missing_original_reference' });
  }
  ...
  try {
    originalAsset = await fetchOriginalAsset({
      supabase,
      originalObjectKey: originalRef.originalObjectKey,
      originalBucket: originalRef.originalBucket,
      originalUrl: originalRef.originalUrl,
      rid: safeJobId,
      diagId,
    });
  } catch (err) {
    if (err?.code === 'original_not_found') {
      logger.warn('prints_original_not_found', {
        diagId,
        requestId,
        originalObjectKey: originalRef.originalObjectKey || null,
        originalUrl: originalRef.originalUrl || null,
      });
      return res.status(404).json({ ok: false, diagId, requestId, reason: 'original_not_found' });
    }
    logger.error('prints_original_fetch_error', {
      diagId,
      requestId,
      message: err?.message || err,
    });
    return res.status(502).json({ ok: false, diagId, requestId, reason: 'original_fetch_failed' });
  }

  const imageBuffer = originalAsset?.buffer;
  if (!Buffer.isBuffer(imageBuffer) || !imageBuffer.length) {
    return res.status(500).json({ ok: false, diagId, requestId, reason: 'empty_original' });
  }
  ...
  for (let attempt = 1; attempt <= MAX_GENERATE_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (err?.code === 'suspicious_output') {
        logger.error('pdf_generate_suspicious_output', {
          diagId,
          requestId,
          jobKey,
          attempt,
          detail: err?.detail || err?.data || null,
        });
        return res.status(422).json({ ok: false, diagId, requestId, reason: 'suspicious_output' });
      }
