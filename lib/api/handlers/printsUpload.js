...
function resolveStraightEdges(payload) {
  if (!payload || typeof payload !== 'object') return false;
  return Boolean(
    payload?.straightEdges === true
    || payload?.straight_edges === true
    || payload?.options?.straightEdges === true
    || payload?.options?.straight_edges === true,
  );
}
...
export async function uploadPrintHandler(req, res) {
  ...
  if (req.method !== 'POST') {
    ...
    return sendJson(res, 405, { ok: false, diagId, requestId, reason: 'method_not_allowed' });
  }
  ...
  if (!payload || !Object.keys(payload).length) {
    try {
      ...
      return sendJson(res, 400, { ok: false, diagId, requestId, reason: 'invalid_body' });
    }
  }
  ...
  if (!widthCm || widthCm <= 0 || !heightCm || heightCm <= 0) {
    return sendJson(res, 400, { ok: false, diagId, requestId, reason: 'invalid_dimensions' });
  }
  ...
  try {
    ...
    return sendJson(res, 500, { ok: false, diagId, requestId, reason: 'supabase_init_failed' });
  }
  ...
  if (!imageSource) {
    return sendJson(res, 400, { ok: false, diagId, requestId, reason: 'missing_image_source' });
  }
  ...
  try {
    ...
    return sendJson(res, 400, { ok: false, diagId, requestId, reason: 'image_download_failed' });
  }

  if (!imagePayload?.buffer || !imagePayload.buffer.length) {
    return sendJson(res, 400, { ok: false, diagId, requestId, reason: 'empty_image' });
  }
  ...
  const isStraightEdges = resolveStraightEdges(payload);
  ...
  try {
    ...
    return sendJson(res, 502, { ok: false, diagId, requestId, reason: 'db_lookup_failed' });
  }
  ...
  for (let attempt = 1; attempt <= MAX_GENERATE_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (err?.code === 'pdf_embed_mismatch') {
        return sendJson(res, 500, { ok: false, diagId, requestId, reason: 'pdf_embed_mismatch' });
      }
      if (attempt >= MAX_GENERATE_ATTEMPTS) {
        return sendJson(res, 500, { ok: false, diagId, requestId, reason: 'pdf_generate_failed' });
      }
      continue;
    }
    ...
    try {
      ...
      if (attempt >= MAX_GENERATE_ATTEMPTS) {
        return sendJson(res, 500, { ok: false, diagId, requestId, reason: 'pdf_validate_failed' });
      }
      continue;
    }
  ...
  try {
    ...
    return sendJson(res, statusCode, { ok: false, diagId, requestId, reason });
  }
  ...
  const designNameRaw = typeof payload.designName === 'string'
    ? payload.designName.trim()
    : typeof payload.design_name === 'string'
      ? payload.design_name.trim()
      : '';
  const recordPayload = {
    job_key: jobKey,
    bucket: uploadResult.bucket,
    ...
    slug: safeSlugValue,
    design_name: designNameRaw || null,
    width_cm: widthCm,
    height_cm: heightCm,
    material: safeMaterial,
  ...
  try {
    ...
    return sendJson(res, 502, { ok: false, diagId, requestId, reason: 'db_upsert_failed' });
  }
