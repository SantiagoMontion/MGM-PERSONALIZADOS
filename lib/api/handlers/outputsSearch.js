...
export async function searchOutputFiles({ query } = {}) {
  ...
  const literalQuery = rawQuery.toLowerCase();
  ...
  try {
    ...
    logger.error('outputs_search_env_error', { diagId, message: err?.message || err });
    return {
      status: 500,
  ...
  logger.debug('outputs_search_start', { diagId, query: rawQuery, limit, offset });
  ...
  for (const month of months) {
    ...
    if (error) {
      logger.error('outputs_search_list_error', {
        diagId,
        prefix: month.key,
    ...
    for (const entry of data) {
      ...
      const fullPath = `${month.key}/${entry.name}`.toLowerCase();
      const matchesNormalized = normalizedQuery ? combined.includes(normalizedQuery) : false;
      const matchesLiteral = literalQuery
        ? entry.name.toLowerCase().includes(literalQuery) || fullPath.includes(literalQuery)
        : false;
      if (!matchesNormalized && !matchesLiteral) {
        continue;
      }
      matches.push({
        name: entry.name,
  ...
  const items = await Promise.all(
    paginated.map(async (item) => {
      ...
      if (error) {
        logger.error('outputs_search_signed_url_error', {
          diagId,
          path: item.path,
          status: error?.status || error?.statusCode || null,
          message: error?.message,
  ...
  logger.debug('outputs_search_ok', { diagId, query: rawQuery, limit, offset, total, returned: items.length });
