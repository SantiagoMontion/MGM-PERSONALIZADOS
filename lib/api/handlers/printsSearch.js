...
function derivePreviewCandidates(pdfPath) {
  const raw = typeof pdfPath === 'string' ? pdfPath.trim() : '';
  if (!raw) return [];
  const withoutOutputs = raw.startsWith('outputs/') ? raw.slice('outputs/'.length) : raw;
  if (!withoutOutputs.startsWith('pdf/')) return [];
  const segments = withoutOutputs.split('/');
  const fileName = segments.pop();
  if (!fileName) return [];
  const baseName = fileName.replace(/\.pdf$/i, '');
  if (!baseName) return [];
  const folder = segments.join('/');
  return ['jpg', 'png'].map((ext) => {
    const suffix = `${baseName}.${ext}`;
    return folder ? `${PREVIEW_FOLDER}/${folder}/${suffix}` : `${PREVIEW_FOLDER}/${suffix}`;
  });
}

const previewExistenceCache = new Map();

async function previewExists(storage, path, diagId) {
  if (!path) return false;
  if (previewExistenceCache.has(path)) {
    return previewExistenceCache.get(path);
  }
  ...
  const promise = (async () => {
    ...
    try {
      const { data, error } = await storage.list(folder, { limit: 1, search: fileName });
      if (error) {
        logger.warn('prints_search_preview_lookup_failed', { diagId, path, message: error?.message || error });
        return false;
      }
      return Array.isArray(data) && data.some((entry) => entry?.name === fileName);
    } catch (err) {
      logger.warn('prints_search_preview_lookup_exception', { diagId, path, message: err?.message || err });
      return false;
    }
  })();

  previewExistenceCache.set(path, promise);
  return promise;
}
...
async function searchDatabase({
  ...
  const items = (await Promise.all(
    rows.map(async (row) => {
      ...
      const rawFilePath = typeof row.file_path === 'string' && row.file_path
        ? row.file_path
        : typeof row.file_name === 'string'
          ? row.file_name
          : '';
      const normalizedPath = rawFilePath.startsWith('outputs/') ? rawFilePath : `outputs/${rawFilePath}`;

      let previewUrl = null;
      const previewCandidates = derivePreviewCandidates(normalizedPath);
      for (const candidate of previewCandidates) {
        // candidate is relative to bucket (e.g. preview/YYYY/MM/file.jpg)
        // eslint-disable-next-line no-await-in-loop
        const exists = await previewExists(storage, candidate, diagId);
        if (exists) {
          previewUrl = buildPreviewUrl(candidate);
          break;
        }
      }
