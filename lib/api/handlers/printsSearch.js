...
async function searchDatabase({
  ...
  const items = (await Promise.all(
    rows.map(async (row) => {
      ...
      const previewUrlRaw = typeof row.preview_url === 'string' ? row.preview_url.trim() : '';
      const previewPathRaw = typeof row.preview_path === 'string' ? row.preview_path.trim() : '';
      let previewUrl = null;
      if (previewUrlRaw) {
        previewUrl = previewUrlRaw.startsWith('/api/')
          ? previewUrlRaw
          : buildPreviewUrl(previewUrlRaw);
      } else if (previewPathRaw) {
        previewUrl = buildPreviewUrl(previewPathRaw);
      }
