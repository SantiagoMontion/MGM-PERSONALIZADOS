...
function resolvePreviewUrl(row) {
  ...
  if (!previewCandidate) {
    ...
    const normalizedPdfPath = filePathCandidate
      ...
      .replace(new RegExp(`^${escapeRegExpLiteral(OUTPUT_BUCKET)}/`, 'i'), '');
  ...
  if (!new RegExp(`^${escapeRegExpLiteral(PREVIEW_BUCKET)}/`, 'i').test(normalizedKey)) {
...
async function searchDatabase({
  ...
  const orFilters = [
    `file_name.ilike.${quotedPattern}`,
    `slug.ilike.${quotedPattern}`,
    `file_path.ilike.${quotedPattern}`,
  ];
  if (orFilters.length) {
