...
async function searchStorageFallback({
  ...
  const storage = supabase.storage.from(OUTPUT_BUCKET);
  const emergencyPrefix = 'pdf-2026-02';
  console.log('[DEBUG SEARCH] Escaneando carpeta específica: pdf-2026-02');

  const emergencyEntries = [];
  try {
    const { data, error } = await storage.list(emergencyPrefix, {
      limit: 100,
      sortBy: { column: 'name', order: 'desc' },
    });
    ...
    if (error) {
      ...
      for (const entry of Array.isArray(data) ? data : []) {
        if (!entry || typeof entry.name !== 'string' || !entry.name) continue;
        const filePath = `${emergencyPrefix}/${entry.name}`;
    ...
    logger.error('prints_search_error', {
      ...
      type: 'storage_list_failed',
      prefix: emergencyPrefix,
      message: err?.message || 'No se pudo listar el bucket.',
    });
  }

  const fallbackPrefixes = buildRecentPdfPrefixes(12);
  const allFiles = await listStorageTree(storage, fallbackPrefixes, diagId);
  const treeFiles = Array.isArray(allFiles) ? allFiles : [];

  const matchesEmergency = emergencyEntries.filter((entry) => {
  ...
  const matchesTree = treeFiles.filter((entry) => {
    const fileName = String(entry.fileName || '').toLowerCase();
    const filePath = String(entry.filePath || '').toLowerCase();
    return fileName.includes(normalizedQuery) || filePath.includes(normalizedQuery);
  });

  const prioritizedMatches = [...matchesEmergency].sort((a, b) => {
    if (sortBy === 'file_name') {
  ...
  const prioritizedPaths = new Set(prioritizedMatches.map((entry) => entry.filePath));
  const nonPrioritizedMatches = matchesTree.filter((entry) => !prioritizedPaths.has(entry.filePath));

  const sorted = [...nonPrioritizedMatches].sort((a, b) => {
    if (sortBy === 'file_name') {
      return String(a.fileName).localeCompare(String(b.fileName));
    }
    const aTime = Date.parse(a.createdAt || '') || 0;
  ...
  const combinedMatches = [...prioritizedMatches, ...sorted];
  const paginated = combinedMatches.slice(offset, offset + limit);
  const items = (await Promise.all(
