...
async function discoverPdfObjectPathByBasename(supabase, bucket, fileName) {
  const target = pdfBasenameFromName(fileName).toLowerCase();
  if (!target) return null;

  const storage = supabase.storage.from(bucket);
  for (const prefix of buildRecentStoragePrefixes(24)) {
    try {
...
async function resolveDownloadUrl(supabase, row, fileName) {
  ...
  const discoveredByBasename = await discoverPdfObjectPathByBasename(supabase, OUTPUT_BUCKET, fileName);
