...
export async function searchPrintsHandler({ query, headers } = {}) {
  ...
  const items = (await Promise.all(
    rows.map(async (row) => {
      ...
      const previewCandidate = typeof row.preview_url === 'string' ? row.preview_url : '';
      const effectivePath = previewCandidate.startsWith('/api/') ? row.file_path || '' : (previewCandidate || row.file_path || '');
      ...
      return {
        ...
        previewUrl: buildPreviewUrl(normalizedPath),
        downloadUrl,
  ...
  logger.debug('prints_search_request/ok', {
    diagId,
    query: rawQuery,
    limit,
    offset,
