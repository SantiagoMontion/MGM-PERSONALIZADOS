...
function buildRecentStoragePrefixes(monthsBack = 24) {
  const prefixes = [];
  const now = new Date();
  for (let i = 0; i < monthsBack; i += 1) {
    ...
    prefixes.push(`pdf/${y}/${m}`);
    prefixes.push(`pdf-${y}-${m}`);
  }
  return prefixes;
}

function pdfBasenameFromName(fileName) {
  const base = String(fileName || '').trim().split(/[/\\]/).pop() || '';
  if (!base) return '';
  return base.toLowerCase().endsWith('.pdf') ? base : `${base}.pdf`;
}

function storagePrefixesFromPathHint(filePath) {
  const hints = [];
  const path = String(filePath || '');
  const dash = path.match(/pdf-(\d{4})-(\d{2})(?:\/|$)/i);
  if (dash) {
    hints.push(`pdf/${dash[1]}/${dash[2]}`);
    hints.push(`pdf-${dash[1]}-${dash[2]}`);
  }
  const slash = path.match(/pdf\/(\d{4})\/(\d{2})(?:\/|$)/i);
  if (slash) {
    hints.push(`pdf/${slash[1]}/${slash[2]}`);
    hints.push(`pdf-${slash[1]}-${slash[2]}`);
  }
  return [...new Set(hints)];
}

/** Busca el PDF en carpetas recientes cuando `file_path` en DB no coincide con Storage. */
async function discoverPdfObjectPathByBasename(supabase, bucket, fileName, filePathHint = '') {
  const target = pdfBasenameFromName(fileName).toLowerCase();
  if (!target) return null;

  const storage = supabase.storage.from(bucket);
  const prefixes = [
    ...storagePrefixesFromPathHint(filePathHint),
    ...buildRecentStoragePrefixes(12),
  ];
  const seen = new Set();
...
async function signDownloadUrlForPaths(supabase, bucketsToTry, paths, fileName) {
  let lastError = null;
...
async function resolveDownloadUrl(supabase, row, fileName, options = {}) {
  const { discoverInStorage = false } = options;
  ...
  const pathVariants = path
    ? [...new Set(pdfStoragePathVariants(path).map(normalizeStorageObjectKey).filter(Boolean))]
    : [];

  if (pathVariants.length) {
    const { url, error } = await signDownloadUrlForPaths(supabase, bucketsToTry, pathVariants, fileName);
    if (url) return url;

    if (discoverInStorage) {
      const discoveredFromPath = await discoverPdfObjectPathByBasename(
        supabase,
        OUTPUT_BUCKET,
        path.split('/').pop() || fileName,
        row.file_path,
      );
      if (discoveredFromPath && !pathVariants.includes(discoveredFromPath)) {
        const retry = await signDownloadUrlForPaths(supabase, bucketsToTry, [discoveredFromPath], fileName);
        if (retry.url) return retry.url;
      }
    }
    ...
    logger.warn('prints_download_sign_failed', {
      ...
      last_error: errMsg,
      discover_in_storage: discoverInStorage,
    });
  }

  if (discoverInStorage) {
    const discoveredByBasename = await discoverPdfObjectPathByBasename(
      supabase,
      OUTPUT_BUCKET,
      fileName,
      row.file_path,
    );
    if (discoveredByBasename) {
      const { url } = await signDownloadUrlForPaths(supabase, bucketsToTry, [discoveredByBasename], fileName);
      if (url) return url;
    }

    logger.warn('prints_download_sign_not_found', {
      file_name: fileName,
      file_path: row.file_path,
      bucket: row.bucket,
    });
...
export { resolveDownloadUrl, resolveFileName };
