...
function toComparableTime(value) {
  ...
  if (value instanceof Date) {
    const directTime = value.getTime();
    return Number.isFinite(directTime) ? directTime : Number.NEGATIVE_INFINITY;
  }
  ...
  const items = (await Promise.all(
    paginated.map(async (entry) => {
      ...
      return {
        ...
        previewUrl: resolvePreviewUrl({ file_path: entry.filePath }),
        downloadUrl,
        url: downloadUrl,
        expiresIn: SEARCH_SIGNED_URL_TTL_SECONDS,
      };
...
function mergeAndSortResults(dbItems = [], storageItems = [], sortBy = 'created_at') {
  ...
  const register = (item) => {
    ...
    dedup.set(key, {
      ...
      createdAt:
        toComparableTime(existing.createdAt) >= toComparableTime(item.createdAt)
          ? existing.createdAt
          : item.createdAt,
    });
  };
