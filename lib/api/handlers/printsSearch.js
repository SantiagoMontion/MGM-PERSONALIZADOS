...
async function resolveDownloadUrl(supabase, row, fileName) {
  ...
  const path = normalizePdfPathForSignedUrl(OUTPUT_BUCKET, row.file_path)
  ...
  const pathVariants = pdfStoragePathVariants(path);
  ...
  logger.debug('prints_download_sign_exhausted', {
