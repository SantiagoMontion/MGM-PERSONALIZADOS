...
async function fetchOnlineStorePublicationId() {
  ...
  if (graphQLErrors.length) {
    try {
      logger.error('online_store_publication_query_errors', {
  ...
  try {
    logger.debug('online_store_publications_count', {
      total: nodes.length,
      source: 'graphql',
    });
  } catch {}
  ...
  if (match?.id) {
    ...
    try {
      logger.debug('online_store_publication_selected', {
        id: publicationId,
  ...
  try {
    logger.warn('online_store_publication_missing', { total: nodes.length });
  } catch {}
...
async function fetchOnlineStorePublicationIdRest() {
  try {
    ...
    try {
      logger.error('online_store_publication_rest_error', {
...
export async function publishToOnlineStore(productGid, publicationIdOverride, options = {}) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      logger.debug('publish_to_online_store_attempt', { attempt, publicationId });
    } catch {}
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (isTransientStatus(resp.status) && attempt < maxAttempts) {
          ...
          try {
            logger.error('publish_to_online_store_error', {
              attempt,
              publicationId,
              status: resp.status,
        ...
        if (graphQLErrors.length) {
          ...
          if (userErrors.length) {
            ...
            try {
              logger.debug('publish_to_online_store_success', {
                attempt,
                publicationId,
                requestId: requestId || null,
              });
