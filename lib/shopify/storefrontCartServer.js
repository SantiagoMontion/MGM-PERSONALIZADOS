...
const STORE_ORIGIN = (process.env.SHOPIFY_CART_ORIGIN || 'https://notmid.ar').replace(/\/+$/, '');
...
async function fetchVariantProductHandle(variantGid) {
  ...
  if (text) {
    try {
      ...
      try {
        logger.error('[storefront_cart] admin_variant_handle_parse_failed', {
          message: err?.message,
          contentType,
  ...
  if (!resp.ok) {
    try {
      logger.error('[storefront_cart] admin_variant_handle_http_error', {
        status: resp.status,
        contentType,
...
async function executeStorefrontMutation(query, variables, { buyerIp } = {}) {
  ...
  try {
    ...
    try {
      logger.error('[storefront_cart] storefront_response_parse_failed', {
        status: resp.status,
        contentType,
        bodyPreview: text ? text.slice(0, 200) : '',
        requestId,
  ...
  if (!resp.ok) {
    try {
      logger.error('[storefront_cart] storefront_http_error', {
        status: resp.status,
        contentType,
        bodyPreview: text ? text.slice(0, 200) : '',
        requestId,
  ...
  if (!json || typeof json !== 'object') {
    try {
      logger.error('[storefront_cart] storefront_invalid_response', {
        status: resp.status,
        contentType,
        bodyPreview: text ? text.slice(0, 200) : '',
        requestId,
...
export async function fallbackCartAdd({ variantNumericId, quantity, jar = new SimpleCookieJar() }) {
  ...
  const headers = {
    ...
    'User-Agent': 'mgm-cart/1.0 (+https://notmid.ar)',
  };
  ...
  try {
    logger.debug('ajax_cart_attempt', {
  ...
  if (text) {
    try {
      ...
      json = null;
      try {
        logger.warn('[storefront_cart] ajax_cart_parse_failed', {
          message: err?.message,
          preview: text.slice(0, 200),
        });
      } catch {}
  ...
  if (!resp.ok) {
    ...
    try {
      logger.error('[storefront_cart] ajax_cart_http_error', {
        status: resp.status,
        contentType,
        bodyPreview: text ? text.slice(0, 200) : '',
        payload,
  ...
  if (!matchedItem) {
    try {
      logger.error('[storefront_cart] ajax_cart_silent_failure', {
        status: resp.status,
        contentType,
        payload,
        detail,
  ...
  try {
    logger.debug('[storefront_cart] ajax_cart_success', {
      status: resp.status,
      contentType,
      cartUrl,
