...
export function buildCartPermalink(variantNumericId, quantity) {
  ...
  return `https://notmid.ar/cart/${variantNumericId}:${qty}`;
}
...
export async function precheckVariantAvailability(
  ...
  for (let i = 0; i < boundedAttempts; i += 1) {
    ...
    try {
      logger.debug(`variant_poll attempt ${attempts}`, {
        availableForSale,
...
export async function createStorefrontCart({
  ...
  const normalizedLineAttributes = mergeAttributes(collectCustomAttributes(lineAttributes));
  const line = {
    merchandiseId: variantGid,
    quantity: qty,
  };
  if (normalizedLineAttributes.length) {
    line.attributes = normalizedLineAttributes;
  }
  const lines = [line];
